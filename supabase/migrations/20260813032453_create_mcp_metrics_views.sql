...
--
-- 지표 정의는 comment로 코드에 남긴다(Supabase Studio가 Description으로 읽는다).
-- 뜻·방향은 적되 경계값(예: "0.3을 넘으면 부실")은 안 적는다 — 지금은 근거가 없고,
-- threshold는 v_search_results로 라벨링해 나중에 정하기로 했다.
...
SELECT '총 던지기' AS metric, total_ingests::numeric AS value,
       'sources 행 수' AS "무엇을 뜻하나", NULL::text AS "높아지면"
FROM stats
UNION ALL
SELECT '총 꺼내기', total_searches::numeric,
       'mcp_tool_calls(tool=search) 행 수', NULL
FROM stats
UNION ALL
SELECT '꺼내기 / 던지기',
...
FROM stats
UNION ALL
SELECT '원문 보기', total_source_views::numeric,
...
FROM stats
UNION ALL
SELECT '평균 최고점수', round(avg_top_score::numeric, 2),
       '검색이 얼마나 확신하나',
...
-- =============================================================
-- v_search_log — 꺼내기 한 건씩. 날짜로 group by 하면 습관이 붙는지 보이고,
-- 결과수=0으로 거르면 못 찾은 쿼리만 남는다.
-- =============================================================
...
SELECT
  created_at AS "시각",
  detail->>'query' AS "쿼리",
  jsonb_array_length(detail->'results') AS "결과수",
  (
    SELECT max((r->>'score')::double precision)
    FROM jsonb_array_elements(detail->'results') r
  ) AS "최고점",
...
COMMENT ON VIEW v_search_log IS
  '꺼내기 한 건씩. 결과수=0으로 거르면 못 찾은 쿼리만 모인다. 시각으로 group by 하면
   추이가 나온다(습관이 붙는지).';
COMMENT ON COLUMN v_search_log."결과수" IS
  '이 쿼리가 실제로 돌려받은 결과 개수(요청 limit이 아니라 반환된 것). 0이면
   재현율 문제 후보 — v_search_log에서 결과수=0으로 걸러 읽는다.';
COMMENT ON COLUMN v_search_log."최고점" IS
  '이 쿼리가 찾은 가장 가까운 결과의 점수. 낮게만 깔리면 임베딩 텍스트를 의심한다.';
COMMENT ON COLUMN v_search_log."최저점" IS
  '이 쿼리가 찾은 결과 중 가장 먼 것의 점수. 최고점과 벌어질수록 결과 안에서도
   확신 편차가 크다는 뜻.';
...
SELECT
  c.created_at AS "시각",
  c.detail->>'query' AS "쿼리",
  r.ordinality AS "순위",
  (r.elem->>'score')::double precision AS "점수",
  d.title AS "다이제스트 제목",
  d.type AS "다이제스트 유형"
