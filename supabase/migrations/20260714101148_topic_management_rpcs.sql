...
-- 참고). 2) create가 없다 — Topic은 사람이 수동 생성하지 않고 리뷰 확정
-- (confirm_ingestion_review·confirm_digest_edit)의 find-or-create가 유일한
-- 생성 경로다. 3) title 옆에 재사용 판단 기준(description)이 없다 — 이름
-- 하나만 고친다.
--
-- 알려진 한계: 위 find-or-create들은 ON CONFLICT (space_id, name) DO UPDATE로
-- status를 건드리지 않는다 — 아카이브된 이름으로 재생성을 시도하면 status는
-- archived로 남은 채 새 Digest 링크만 조용히 붙는다(Tag의 create_tag는 이
-- 경우를 막고 restore를 안내하지만, find-or-create 경로는 그 가드를 거치지
-- 않는다). Tag에도 이미 있는 결함이라 이 PR에서 함께 고치지 않는다 — 별도
-- 트래킹 필요.
-- =============================================================
...
--   상태 가드 실패는 NM005("그 사이 상태가 바뀜" — source의 NM004와 같은
--   결이지만 엔티티가 달라 코드를 나눈다)로 던져 error-mapper가 예상된 도메인
--   오류로 분류하게 한다. UNIQUE(space_id, name)은 archived 행도 포함하므로,
--   다른 활성 주제와 이름이 겹치면 제약 이름을 확인한 뒤 NM006으로 다시 던져
--   명확한 에러를 준다(원시 unique_violation 그대로 두면 향후 다른 unique
--   제약과 코드를 공유하게 된다 — create_space와 같은 결).
-- =============================================================
