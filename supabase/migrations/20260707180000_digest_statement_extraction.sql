...
--
-- 07-modeling 확정 동작: 리뷰 확정(1단계)이 원본을 active로 밀면, 추출(2단계)이
-- 그 원본의 확정된 Digest들을 읽어 Statement를 뽑는다. Statement는 Source가 아니라
-- 자기가 나온 Digest를 digest_id로 가리키고(확정 후 안 바뀌는 Digest라 근거가 안정적),
-- 그 create는 confirm이 만든 **같은 ingestion changeset에 이어 붙는다**(2단계는 같은
-- 인제스천의 연속 — 원본 빼기 되돌리기가 Digest·Reference·진술을 하나로 닫으려면
-- 셋이 한 changeset에 있어야 한다).
--
-- 1) apply_ingestion_changeset → apply_extraction_statements 재작성:
--    새 changeset을 만들지 않고, 그 원본의 applied ingestion changeset에
--    create statement change를 append + statements.digest_id 채움.
-- 2) service_role에 digests SELECT — 워커가 원본의 확정 Digest를 인출한다.
-- =============================================================

-- =============================================================
-- 1) apply_extraction_statements — 진술을 원본의 ingestion changeset에 덧붙인다
--
-- p_statements 원소:
--   { "content", "type", "confidence"|null, "digest_id", "index", "due_date"|null }
--   - digest_id: 이 진술이 나온 Digest (추출 근거, statements.digest_id로 저장)
--   - index: 원문 등장 순서(원본 전체 관통). locator로 저장 — 잇기 정렬이 쓴다.
-- =============================================================
...
BEGIN
  ...
  -- 진술을 붙일 곳 = confirm이 만든 그 원본의 applied ingestion changeset.
  -- J는 원본 1개 = ingestion changeset 1개. O(Digest 수정)에서 원본에 changeset이
  -- 여럿(ingestion + manual)이 되면 여기를 "진술의 digest를 만든 changeset"으로
  -- 일반화한다 — 지금은 그 확장을 막지 않도록 원본 단위로만 좁힌다.
  ...
  LOOP
    ...
    -- 보증한다 — 워커가 원본의 digest만 넘기므로 소속 검증은 워커 몫.
...
-- =============================================================
-- 2) digests SELECT — 워커가 원본의 확정 Digest(body)를 추출 입력으로 인출한다.
