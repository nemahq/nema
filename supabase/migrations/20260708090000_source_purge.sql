-- =============================================================
-- 원본 완전 삭제(purge) — 보관기간 지난 trashed Source를 배치로 hard delete
--
-- 07-modeling §완전 삭제: trashed_at + 보관기간(30일)이 지난 원본을 배치가 완전
-- 삭제한다. 이 슬라이스에서 확정한 설계:
--   - purge = `DELETE FROM sources` 한 번. 파생물(Digest·진술·관계·그 원본의
--     changeset·change)은 이미 걸린 source_id CASCADE로 함께 사라진다. Reference는
--     workspace_id에 매달려 원본 삭제 그물에 안 걸려 생존한다 — Workspace 공유
--     자원이라 다른 원본이 인용 중일 수 있어 지우면 안 된다. revert·cross-source가
...
-- 막아 원본→Digest CASCADE를 튕긴다. 이걸 CASCADE로 바꾸면 원본 삭제가
...
--    원본→Digest→진술 연쇄를 막던 NO ACTION을 푼다. Digest 삭제는 원본 완전 삭제와
...
--    vector_purge에 예약한 뒤, 원본을 DELETE한다. 나머지는 source_id CASCADE가
--    처리한다. 반환값은 이번에 지운 원본 수.
