...
-- 나란히 뜨면 같은 사실을 두 번 말하는 꼴이다. 그래서 먼저 이어진 하나만 남는다 —
-- 어느 갈래가 먼저 가느냐는 앱이 정한다(services/source-service.ts).

COMMENT ON TABLE digest_relations IS
  '다이제스트 사이의 방향 있는 관계. 지지·약화는 from이 하는 쪽, to가 받는 쪽이라
   "from이 to를 지지/약화한다"로 읽는다. 중복·충돌은 논리적으로 대칭이라 양 끝을
   뒤집어도 뜻이 같고, from에 새로 온 쪽을 둔다. 넷 다 아무것도 접지 않아
...
COMMENT ON COLUMN relation_judgments.judgment IS
  '이 판정이 물은 질문 묶음(= 갈래). support_weaken | duplicate_conflict —
   relation-rules.ts의 RelationJudgmentName과 같은 값이다.';

-- 20260813121456의 "다이제스트 하나 = 1행"이 이 마이그레이션으로 거짓이 된다.
COMMENT ON TABLE relation_judgments IS
  '관계 판정 한 건씩(다이제스트 하나 × 갈래 하나 = LLM 1콜 = 1행). candidates는
   [{ digestId, score, verdict }] — score는 판정에 넘길 때의 벡터 유사도,
   verdict는 판정 결과(관계 종류 또는 none)다. 후보가 0개면 LLM을 안 부르므로
   행도 안 생긴다 — 갈래마다 후보 범위가 달라 한쪽만 행이 남는 다이제스트가 흔하다.';

-- 뷰에도 실어 갈래별로 갈라 볼 수 있게 한다. CREATE OR REPLACE VIEW는 기존 컬럼
-- 순서를 못 바꾼다 — 새 컬럼은 끝에 붙인다.
CREATE OR REPLACE VIEW v_relation_candidates WITH (security_invoker = true) AS
SELECT
