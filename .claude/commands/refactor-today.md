...
2. **Loop** until no meaningful unit remains in the pool:
   ...
   6. `gh pr create` with **base = the previous branch in the chain** (or `staging` for the first unit) — this is what makes each PR show only its own incremental diff. Follow `.github/pull_request_template.md`. Title in Korean. Label `refactoring`. Reviewer: the relevant original PR's author. Why: name which of today's PRs this unit cleans up after. What: key design decisions only.
