...
const AssistDraftInputSchema = DraftAssistInputSchema.omit({ origin: true });
...
export function registerAssistDraft(server: McpServer): void {
  server.registerTool(
    "assist_draft",
    {
      ...
      inputSchema: AssistDraftInputSchema.shape,
      annotations: {
        readOnlyHint: false,
    ...
    async (input, extra) => {
      const result = await authedClient(extra).draft.assist.mutate({
        origin: "external",
        ...input,
      });
      return {
