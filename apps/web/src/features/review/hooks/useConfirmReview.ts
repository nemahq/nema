...
// changeset.getByNumber를 무효화하는 이유: 모든 상태가 URL을 공유해서(changesetDetailRegistry), 확정 성공 후 이 쿼리가 새 status(applied)로 다시 읽히기만 하면
// 같은 URL이 자연히 ChangesetRecordScreen으로 넘어간다 — 별도 navigate가 필요 없다.
// 이 invalidate 자체가 실패하면(네트워크 등) 확정은 이미 성공했는데 화면만 편집 상태에
