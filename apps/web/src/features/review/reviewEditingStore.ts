...
export type ReviewEditingAction =
  ...
  // 이 id를 쓰는 모든 Digest에 적용되는 전역 액션 — digest/setTags처럼 하나의
  // digest id를 받지 않는다(reviewEditingState.ts의 tagRenames/topicRenames 주석 참고).
  | { type: "tag/renamed"; id: string; title: string; description: string }
  | { type: "topic/renamed"; id: string; title: string }
  | { type: "reference/set"; id: string; reference: ReviewNewReference }
...
export function reviewEditingReducer(
  ...
  switch (action.type) {
    ...
    case "digest/setBodyField": {
      const current = overrides.bodyOverrides.get(action.id) ?? action.baseBody;
      ...
      return {
        ...overrides,
        bodyOverrides: new Map(overrides.bodyOverrides).set(action.id, next),
      };
    }
    ...
    case "digest/remove":
      return {
        ...overrides,
        removedDigestIds: new Set(overrides.removedDigestIds).add(action.id),
      };
    case "tag/renamed":
      return {
        ...overrides,
    ...
    case "reference/remove":
      return {
        ...overrides,
        removedReferenceIds: new Set(overrides.removedReferenceIds).add(
          action.id,
        ),
      };
