...
import { useDeleteWaitingDrafts } from "@web/features/drafts/hooks/useDeleteWaitingDrafts";
...
export function DeleteWaitingDraftsDialog({
  ...
  async function handleDeleteAll() {
    try {
      ...
      if (failedCount > 0) {
        toast.error(
          t("draft.delete_waiting_partial_failure", { failed: failedCount }),
        );
      }
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false}>
        <DialogHeader>
          <DialogTitle>
            {t("draft.delete_waiting_confirm_title", {
