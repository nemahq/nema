...
- Duplicate public `VITE_*` config in Railway. `.env.{staging,production}` is the single source of truth — Railway provides only secrets (`SENTRY_*`), the build-mode selector (`VITE_APP_ENV`), and platform-injected vars. Duplicating a `VITE_*` value in Railway lets it silently override the repo at build time and drift.
