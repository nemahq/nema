...
function fakeSupabase(rows: unknown[]): TypedSupabaseClient {
  mockReturns.mockReset().mockResolvedValue({ data: rows, error: null });
  const from = vi.fn().mockReturnValue({
    select: vi.fn().mockReturnValue({
      in: vi.fn().mockReturnValue({ returns: mockReturns }),
    }),
  });
...
describe("searchDigests", () => {
  ...
  // 고아 벡터가 남을 수 있다 — 그 벡터가 검색 결과로 돌아오지 않도록 v_visible_digests
  // (3단 상속 판정)를 반드시 거쳐야 한다. .from()이 digests 테이블로 바뀌거나
  // 다른 테이블로 바뀌어도 이 단언이 잡는다.
  it("가려진 다이제스트를 DB에서 뺄 때 v_visible_digests를 통해 읽는다", async () => {
    ...
    expect(supabase.from).toHaveBeenCalledWith("v_visible_digests");
  });
