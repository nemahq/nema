...
async function fetchDigestSnapshots(args: {
  ...
  const { data: rows, error } = await supabase
    .from("digests")
    .select(
      "id, title, description, body, external_urls, author_id, author_name, status, created_at, digest_topics(topic:topics(id, title)), digest_tags(tag:tags(id, title, description, color)), digest_references(reference_id)",
    )
    .in("id", digestIds);
