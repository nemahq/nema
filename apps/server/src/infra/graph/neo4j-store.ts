...
export function createNeo4jStore(): GraphStore & { close(): Promise<void> } {
  ...
  return {
    async ensureSchema(): Promise<void> {
      ...
      try {
        ...
        await session.run(`
          ...
          WITH keep, dup
          CALL (keep, dup) {
            MATCH (dup)-[:MENTIONED_IN]->(d:Document)
            MERGE (keep)-[:MENTIONED_IN]->(d)
            UNION
            MATCH (dup)-[:RELATED_TO]-(other:Entity)
            WHERE other <> keep
    ...
    async findDocumentsByEntities(
      ...
      const { entities, userId, limit = 10 } = options;
      if (entities.length === 0) {
        return [];
      }
      ...
      try {
        const result = await session.run(
          `MATCH (e:Entity {userId: $userId})-[:MENTIONED_IN]->(d:Document)
           ...
           LIMIT $limit`,
          { userId, entities, limit: neo4j.int(limit) },
        );
      ...
      try {
        const result = await session.run(
          `UNWIND $queries AS q
           ...
           RETURN type, normalizedName, e.name AS name`,
          { userId, queries },
        );
        return result.records.map((r) => ({
          type: getEntityType(r, "type"),
