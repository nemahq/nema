...
-- "authorId는 사람 삭제와 무관하게 콘텐츠를 보존한다"). 문제는 이름을 별도로
...
BEGIN
  ...
  INSERT INTO sources (
    space_id, author_id, author_name, body, author_timezone, status,
    digestion_started_at
  )
  VALUES (
    p_space_id, v_author_id, resolve_user_display_name(v_author_id), p_body, p_author_timezone, 'pending',
    now()
  )
...
-- =============================================================
-- confirm_ingestion_review — Digest는 Source 제출자의 이름 스냅샷을 승계
-- (author_id 승계와 같은 관용구 — sources.author_name을 그대로 복사할 뿐,
-- 확정 시점에 다시 계산하지 않는다. Source가 이미 제출 시점 스냅샷을 들고
-- 있으므로 그 값이 "생성 시점 이름"의 진짜 출처다)
-- =============================================================
...
-- =============================================================
-- confirm_digest_edit — 편집을 제출한 사람이 changeset·새 Digest 둘 다의
-- author. 이름도 그 시점에 함께 스냅샷.
-- =============================================================
...
-- =============================================================
-- resolve_duplicate_relation — 병합을 확정한 사람이 새 Digest의 author
-- (이 changeset 자체는 엔진 산물(type='relation')이라 author_id/author_name을
-- 안 채우는 기존 규칙 그대로 — 여기서 바뀌는 건 새로 생기는 Digest 쪽뿐이다)
-- =============================================================
...
-- =============================================================
-- update_reference / archive_reference — 편집·아카이브한 사람이 changeset의 author
-- =============================================================
...
BEGIN
  ...
  INSERT INTO changesets (space_id, type, status, author_id, author_name)
  VALUES (NULL, 'manual', 'applied', v_author_id, resolve_user_display_name(v_author_id))
  RETURNING id INTO v_changeset_id;
...
-- =============================================================
-- revert_changeset — 되돌리기 버튼을 누른 사람이 revert changeset의 author
-- =============================================================
...
BEGIN
  ...
  INSERT INTO changesets (space_id, type, status, reverts_id, author_id, author_name, title, revert_depth)
  VALUES (
    v_space_id, 'revert', 'applied', p_changeset_id, v_author_id, resolve_user_display_name(v_author_id),
...
-- =============================================================
-- list_manual_changes_for_target — author_name도 함께 반환(변경 이력 모달 표시용).
-- 반환 컬럼이 늘어나 시그니처가 바뀌므로 DROP 후 재생성한다.
-- =============================================================

DROP FUNCTION IF EXISTS list_manual_changes_for_target(change_target_type, uuid);
...
BEGIN
  ...
  RETURN QUERY
  SELECT ch.id, ch.changeset_id, cs.number, cs.author_id, cs.author_name, ch.created_at, ch.action, ch.data
  FROM changes ch
