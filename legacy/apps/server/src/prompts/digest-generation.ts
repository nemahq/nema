...
- When the note clearly introduces a NEW recurring entity worth a registry entry, add it to the top-level "newReferences" with a short key you make up (e.g. "R1"), its type ("person" | "organization" | "project" | "product" | "term"), its name as "title", and a "body" that captures what the note says about it. If a digest actually discusses it, also cite that key in that digest's "newReferenceKeys" — but citation is not what earns it a place in "newReferences"; the note having introduced it is. "organization" is an acting entity (a company, a team); "product" is a thing an entity makes — they are different registry entries.
