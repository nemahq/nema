...
- **정한다**: coarse 단계의 매칭 방식(LLM 선택), 그 자리(구조화 레이어에 합침), recall 우선 설계와 우아한 강등, 무태그 원문 처리, 사용자 대면 모드(수동 제거: 그냥 질문), coarse recall의 eval 구조.
...
- **원문은 주제 0개도 된다**(무태그 허용: draft-assist가 "애매하면 라벨 말고 비워라"). → scope의 구조적 구멍(§4).
