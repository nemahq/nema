...
> ⚠️ **대체됨 (PR I-a·I-b).** 이 문서가 세운 초안(draft) 스테이징 층은 v2 Digest 파이프라인이 대체했다. 원문은 곧바로 pending으로 박제되고(`create_source`), 워커가 Digest 후보를 생성해 리뷰 게이트(`digestReview`)에 세우며, 사람이 확정하면 추출이 시작된다. 아래 draft 엔티티·RPC·MCP draft 도구 서술은 역사적 기록이다 — 현재 모델은 `docs/foundations/07-modeling.md`가 SSOT다.
>
...
- 확정하면 초안은 사라지고 내용은 원문(기억)으로 승격한다. 폐기(버리기) 허용.
...
- 주제는 원문(source)에 0..N개 붙는다(멀티 라벨, 무태그 허용). 진술의 주제는 진술 → 원문 → 주제 조회로 따라온다.
- 주제는 공간(Space)에 쌓이는 재사용 목록(레지스트리)이다. 평평한 단일 라벨이고 계층·군집·진술별 표식은 없다.
- 주제 레지스트리 = 지도의 줄기 목록. 같은 주제를 가진 원문들의 집합이 곧 한 줄기다(별도 줄기 엔티티 없음).
...
-- 원문 0..N 주제 (멀티 라벨, 무태그 허용). 진술의 주제는 statement_sources -> source_topics로 파생.
...
설계 근거: 진술별 주제 표식은 추출 워커에 "진술별 주제 분류"라는 새 능력을 심어야 하므로 버린다. 원문 단위 멀티 라벨이면 확정 게이트 한 곳에서 사람이 정하고, 진술은 join으로 따라오며, 엔진은 주제를 몰라도 된다. merge로 한 진술이 여러 원문에 걸려도 "원문들의 주제 합집합"으로 자연스럽게 풀린다.
...
BEGIN
  ...
  -- 무태그 원문(미분류): 의도된 상태다(확정 시 주제 강제 안 함).
...
진술은 자기 주제를 들지 않는다. `statement -> statement_sources -> sources -> source_topics -> topics`로 따라온다. 지도의 줄기 = `topics` 한 행, 그 줄기의 내용 = 그 주제를 가진 원문들과 그 원문의 진술들.

주제 없는 원문(무태그)은 어느 줄기에도 안 잡힌다. 지도는 이들을 "미분류" 묶음으로 따로 보여, 나중에 태깅(교정)할 인박스 역할을 한다. 무태그를 허용하는 값은 확신 없을 때 틀린 주제를 강요하지 않는 데 있다(원칙: 틀릴 만한 건 단정하지 않음 + 사람 주권).
...
2. 신뢰를 올리는가(몰래 안 지우고 되돌릴 수 있나): 확정은 사람 게이트, 충돌·애매는 기존 pending changeset으로 사람에게, 되돌리기·archive 엔진 불변. 확정 시 원문 보존(초안만 사라지고 내용은 source로 승격). 예.
...
- 똑똑한 주제 군집화, 진술별 주제, 진술의 다중 주제 분리. 멀티 라벨은 원문 단위 통째 등장까지만.
- 외부 초안 본문 추가 다듬기(이중 가공 방지). 사람의 직접 수정은 허용(공동 편집), 도우미 재가공만 막음.
- 초안 작성 주고받기 대화(반복 편집). 한 방 제안만 이번 슬라이스.
- 맥락 내보내기(context-out). 별도 슬라이스.
- 확정 후 원문 주제 사후 교정(source_topics 편집)은 후속. 이번엔 확정 게이트에서의 교정까지.

엣지(노트): merge로 한 진술이 여러 원문에 걸리면 주제가 여럿 보일 수 있다. 멀티 라벨 모델에서 의도된 동작(합집합)이라 손해가 아니다.
