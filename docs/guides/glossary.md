...
- **원본 vs 원문**: Source 엔티티 자체(삭제·복원 등 엔티티 단위 액션, "원본 삭제")는 "원본"을 쓴다. Source의 텍스트 내용을 읽는 액션("원문 보기" 버튼, "원문에 없어 못 채운 필드")은 "원문"을 쓴다 — 한국어에서 "원본을 보다"보다 "원문을 보다"가 관용적으로 훨씬 자연스럽고(뉴스 큐레이션·번역 앱의 "원문보기" 관례), "원본을 삭제하다"는 반대로 "원문을 삭제하다"보다 자연스럽다. 영문은 두 경우 다 Source로 통일(구분 없음).
