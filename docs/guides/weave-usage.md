...
## 아이콘 굵기(`strokeWidth`) 3단 위계

아이콘의 무게는 크기 × 상시 노출 여부 × 상태 강조 필요 여부다. `lucide-react` 기본값(2) 하나로는 이 세 문맥을 다 감당할 수 없다.

| 값 | 용도 |
|---|---|
| `2`(기본값, prop 생략) | 본문 속 일반 UI 아이콘 — 버튼·필드·툴바 등 텍스트와 대등하게 나란히 오는 자리 |
| `1.5` | 조용한 문맥 — ① 내비게이션(LNB, 사이드바 토글)처럼 상시 노출되어 존재감을 낮춰야 하는 아이콘, ② `size-10` 이상 큰 empty-state 일러스트(광학 보정 — 큰 아이콘에 기본 굵기를 쓰면 뭉툭해 보인다), ③ `fg-tertiary` 이하 저채도 텍스트 옆에 붙는 구조적 보조 표시(연결선·불릿 등) — 정보가 아니라 배치만 나타내므로 옆 텍스트보다 존재감을 낮춰야 한다 |
| `3` | 상태를 순간적으로 확정 짓는 아주 작은 아이콘(`size-3.5`~`size-4`, 체크마크류) — 작을수록 굵어야 가독성이 유지되고, 선택됨 상태는 강조되어야 한다 |

이 값 외의 임의 굵기(예: `2.5`)는 위 세 문맥 중 어디에도 안 속한다는 뜻이라 위계에서 벗어난다. lucide가 아닌 커스텀 svg도 역할이 위 셋 중 하나에 해당하면 같은 값을 따른다. 판단 기준은 "텍스트와 나란히 있는가"가 아니라 "그 텍스트와 대등한 정보량을 갖는가"다 — `DetailConnectorIcon`처럼 `fg-tertiary` 디테일 문구 옆에 붙는 저채도 연결선은 텍스트와 나란히 있어도 ③에 해당해 `1.5`를 쓴다.

**적용 기준은 린트로 못 잡는다** — "이 아이콘이 본문/조용함/강조 중 뭐냐"는 문맥 판단이라 이 문서와 PR 리뷰가 대신한다.
