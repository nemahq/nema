...
| `1.5` | 조용한 문맥 — ① 내비게이션(LNB, 사이드바 토글)처럼 상시 노출되어 존재감을 낮춰야 하는 아이콘, ② `size-10` 이상 큰 empty-state 일러스트(광학 보정 — 큰 아이콘에 기본 굵기를 쓰면 뭉툭해 보인다) |
| `3` | 상태를 순간적으로 확정 짓는 아주 작은 아이콘(`size-3.5`~`size-4`, 체크마크류) — 작을수록 굵어야 가독성이 유지되고, 선택됨 상태는 강조되어야 한다 |

이 값 외의 임의 굵기(예: `2.5`)는 위 세 문맥 중 어디에도 안 속한다는 뜻이라 위계에서 벗어난다. lucide가 아닌 커스텀 svg도 역할이 위 셋 중 하나에 해당하면 같은 값을 따른다.
