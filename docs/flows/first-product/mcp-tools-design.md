...
`list_topics`는 레일과 함께 이미 깔렸다. 이번에 더하는 것은 신규 12종이다. 모두 같은 형태다. 입력은 `@nema-io/shared` 스키마를 재사용하고, 본문은 대응 tRPC 프로시저를 호출해 결과를 그대로 돌려준다.
...
- 신규 도구 12종 등록. 각 도구는 입력 스키마 재사용 + tRPC 호출 + 결과 반환의 한 형태를 따른다.
