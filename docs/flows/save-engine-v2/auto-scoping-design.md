...
> **한계와 측정 결과**: 이름만 쓰는 coarse는 모호한 질의는 전역으로 잘 강등하지만(측정 #17 강등 칸 recall 1.0), **이름에 안 드러난 디테일 질의에선 엉뚱한 주제로 샌다**(측정 #17 묻힌 사실 0.8). 주제에 한 줄 설명을 달면 그게 메워진다(0.8→1.0). 그래서 v1부터 설명을 켠다(topic-substrate §8). 진술 샘플까지 주는 더 무거운 표현은 여전히 측정으로 게이트.
