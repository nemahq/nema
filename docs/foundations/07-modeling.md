...
- **주제는 사람이 붙이는 1급 분류** — 주제(`topic`)는 개체와 달리, 사람이 확정 때 원본에 붙이는 굵은 분류 라벨이다. 공간별 레지스트리로 영속해 재사용되고(`topics`·`source_topics`), 같은 주제를 가진 원본들이 다시 켰을 때 한 갈래로 모인다. 진술처럼 의미로 다뤄지는 단위는 아니고 원본에 붙는다. 상세는 콘텐츠 입구 설계(`docs/blueprints/first-product/content-intake-design.md`).
- **개체는 단위가 아님** — 인물·조직 같은 개체는 별도 단위가 아니라 진술에서 파생되는 검색용 꼬리표/렌즈다. 단위·소유·변경셋에 들어가지 않고 언제든 재생성된다. 사람이 확정해 영속하는 주제와 달리, 자동으로 뽑히고 저장되지 않는다.
