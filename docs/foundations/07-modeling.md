...
- Source에 `title?` 필드가 필요한가 — 지금 Source는 `body`(원문 텍스트) 외 제목이 없다. 순수 타이핑·붙여넣기 입력은 원래 제목이 없어 문제없지만, Tiro 같은 음성/노트 연동은 원본 쪽에 이미 회의 제목·날짜가 있는데 지금 모델로는 이게 그냥 버려진다. Digest 리뷰 화면은 이 필드 없이도 원문 탭을 기본으로 열어 대체했지만(surface-inventory.md 참고), 다른 화면(예: 초안 목록)에서 Source를 식별 가능한 이름으로 보여줘야 할 때 다시 부딪힐 수 있다.
...
- `Digest.locator`가 연속된 범위 하나만 가리키는지, 여러 군데로 흩어진 범위를 가리킬 수 있는지 — 한 Digest가 원문 여기저기 흩어진 내용을 종합했을 때를 대비해 다중 범위 지원이 필요할 수 있다. Digest 리뷰 화면의 원문 대조(사이드 패널, locator 영역 하이라이트)가 다중 범위를 가정하고 설계됐으니, 실제 추출 엔진이 이를 채울 수 있는지 확인 필요.
- 1단계(Digest 추출)·2단계(Statement·Relation 추출)를 합칠지 — 지금처럼 나누면 Digest를 다듬는 리뷰 시점엔 충돌·관련 정보를 모르고, 확정 후에야 드러난다. 합치면 리뷰 중에 바로 알 수 있지만, 아직 다듬어지지 않은(오타·잘못된 타입 등이 안 고쳐진) Digest 초안을 기준으로 Statement를 뽑게 되어 비교·관계 결과에 노이즈가 낄 위험이 있다. 딱 떨어지는 답이 없어 지금은 나누는 쪽을 유지하고, 별도 라운드에서 트레이드오프를 더 볼 것.
