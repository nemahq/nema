...
- **정리할 내용 없음 draft 상세에 워닝 배너 없음**: "정리할 내용이 없어요." 상태로 처리된 draft 상세 화면 제목 상단에 워닝 배너 문구가 안 뜸. 사용자가 값을 수정할 때까지 배너가 떠 있다가, 기억하기 재시도 시 "정리중..'' 영역으로 바뀌어야 함. 초안 오버뷰의 "정리내용없음" 아이콘과 같은 라이프사이클이어야 함(미조사).
- **Draft 상세 Space 변경 시 과도하게 느림**: draft 상세 상단 Space 셀렉트로 초안 저장 스페이스를 변경할 때 체감 속도가 너무 느림. 네트워크 탭 확인 결과 필요 이상의 요청이 발생하는 것으로 보임(미조사).
- **인제스천 완료 브라우저 알림이 실제 화면 반영보다 이름**: 인제스천 완료 시 "리뷰 필요함" 브라우저 알림이 뜨는 시점이 실제 화면(배지·목록 등)에 반영되는 시점보다 더 이름. 알림 발생 시점 조정 필요(미조사).
- **Digest 모두 삭제 시 자동저장 실패**: Digest 후보를 전부 삭제하면 자동저장이 실패함(미조사).
- **자동저장 문구가 '방금 저장됨'에 고정돼 편집 반영 여부 파악 불가**: 자동저장 상태 문구가 '방금 저장됨'으로 고정 표시돼, 실제로 방금 편집한 내용이 저장된 게 맞는지 구분이 안 됨(미조사).
- **Digest 삭제 후 실행취소 시 렌더링 순서가 바뀜(간헐적)**: Digest 후보 삭제 후 cmd+z로 실행취소하면 후보 자체는 되살아나지만, 카드 목록의 렌더링 순서가 원래와 달라질 때가 있음. 재현해보니 매번 발생하지 않고 간헐적임(미조사).
- **'방금 저장됨' 클릭 시 뜨는 팝오버 UI 고민 필요**: 자동저장 상태 텍스트('방금 저장됨')를 클릭하면 뜨는 팝오버의 UI에 대한 고민이 필요함(미조사).
- **'저장 실패' 상태의 UI/UX 고민 필요**: 자동저장 상태 문구가 '저장 실패'로 바뀌었을 때의 UI/UX가 아직 정리 안 됨 — 이때도 팝오버가 필요한지부터 검토 필요(미조사).
- **변경사항 탭 리스트가 적절한 시점에 최신화 안 됨**: Digest 리뷰 확정 등으로 changeset 상태가 바뀐 후, 변경사항 탭 리스트가 적절한 시점에 최신 상태로 갱신되지 않음(미조사).
- **변경셋 상단 액션 버튼(버리기·되돌리기·확정·되살리기 등) 비활성화 범위 체크 필요**: 클릭 시 처리 중 비활성화되는 범위가 적절한지 확인 필요(미조사).
- **수정 상태가 아닌(확정된) 변경셋 화면 적용 범위와 UI 디테일 체크 필요**: closed(적용됨) 상태 변경셋 화면에서 어디까지가 적용 범위인지, UI 디테일이 적절한지 확인 필요(미조사).
- **"버리기" 레이블을 "반려"로 수정 필요**: 용어 변경 필요(미조사).
- **리뷰 페이지 상단 액션 버튼 로딩 경험 통일 필요**: 버튼별로 로딩 표현이 제각각이라 통일 필요(미조사).
- **반려됨 화면이 만들어져 있지 않음**: 리뷰를 버렸을(반려) 때 이동하는 화면 자체가 없음(미조사).
- **중첩된 되돌리기(revert) 제목에서 따옴표가 최초 1회 이후 안 붙음**: 되돌리기를 다시 되돌리는 등 중첩될 때, 정상이라면 매번 이전 제목 전체를 따옴표로 감싸며 쌓여야 함(예: `""원본" 되돌림" 되돌림`). 현재는 최초 1회만 따옴표가 붙고 그 다음부터는 안 붙어 `"원본" 되돌림 되돌림`처럼 나옴(미조사).
- **이미 한 번 되돌린 changeset에 되돌리기 버튼이 그대로 남아있음**: 이미 되돌려진 changeset에서 되돌리기 버튼이 계속 눌리는 게 맞는지 — 없어지거나 비활성화되어야 할 것으로 보임(미조사, PM 판단 필요 — 07-modeling.md "되돌리기는 append-only" 원칙상 재되돌리기=redo가 허용되는데 이게 원본 changeset에서도 계속 열려있는 게 맞는지 확인 필요).
