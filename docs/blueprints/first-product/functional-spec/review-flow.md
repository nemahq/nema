...
> **2026-08-06 스코프 재정의 반영.** "리뷰"가 두 가지를 가리키고 있었고, 하나만 남았다.
>
> | | 무엇 | 이번 라운드 |
> |---|---|---|
> | **넣기 리뷰** | 넣은 결과를 확인·확정해야 저장됨 | **없앰** |
> | **판정 리뷰** | 충돌·중복을 사람이 결정 | **유지 — 이번 라운드의 핵심** |
>
> 넣기 게이트를 없앤 건 스코프를 줄인 게 아니라 **제품 정의를 바꾼 것**이다. 게이트는 추출 품질이 나쁠 때를 대비한 보험인데, 켜두면 넣을 때마다 마찰을 내면서 정작 품질이 나쁘다는 사실을 사람이 매번 손으로 메워 안 드러나게 만든다 — 이번 분기에 재려는 대상을 가리는 장치였다. 그리고 넣는 시점의 사람은 판정할 근거가 없다("이게 기존 것과 부딪히나"는 대조가 끝나야 알 수 있다). 그래서 검증 지점을 판단이 가능한 자리로 옮겼을 뿐, 검증 자체를 없앤 것이 아니다.
>
> **아래 "Ingestion 리뷰"·"실행취소" 섹션은 그 게이트 위에서 동작하는 것들이라 이번 범위 밖이다.** 체크박스는 만들었고 실동작까지 확인했다는 기록이라 그대로 둔다 — 편집 기능을 넣는 라운드에서 이 목록을 그대로 다시 쓴다.
...
> **이번 범위 밖 — 섹션 전체.** 확정 게이트를 전제한 섹션이다. 게이트를 없애면 인제스천 changeset이 `open`으로 태어나지 않아 이 화면에 도달할 경로 자체가 닫힌다(코드는 그대로 두고 진입만 닫힌다). 체크된 항목은 만들었고 실동작까지 확인했다는 기록이라 그대로 두고, 미체크 항목도 그 자리에 둔다 — 편집 기능을 넣는 라운드에서 이 목록을 통째로 다시 쓴다.
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-28, PR #515·#517)**: `digest-review-service.ts`의 `getReview`가 레지스트리에 매칭 안 되는 Tag를 `registryId: null`(신규)로 후보에 미리 채우고, 확정 시 `confirm_ingestion_review`가 find-or-create(`ON CONFLICT ... DO UPDATE`)로 실제 행을 만든다. 같은 후보 안에서 기존/신규 Tag가 배타적이지 않게 섞일 수 있는 구조. 신규 Tag는 이제 색상도 함께 제안된다("Tag 색상 지정 — 신규 생성 시" 참고).
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-20, QA 세션 / 2026-07-28 PR #515 갱신)**: 이름이 일치하는 `status='active'` Topic은 `registryId`가 채워져 재사용되고, archived 항목은 재사용 후보에서 제외된다(`digest-review-service.ts`).
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-20, QA 세션 / 2026-07-28 PR #515·#517 갱신)**: 이름이 일치하는 `status='active'` Tag는 `registryId`가 채워져 재사용되고, archived 항목은 재사용 후보에서 제외된다(`digest-review-service.ts`). 그 Tag가 이미 가진 색상도 그대로 후보에 실려 온다 — 붙이는 순간 새 랜덤 색이 아니라 기존 색 그대로 보인다("Tag 색상 지정 — 신규 생성 시" 참고).
...
- **범위 참고 (2026-07-27, PR #506·#510 갱신)**: 이전 구현(`EditableLabelChip`의 `readOnly` 인라인 편집)은 걷어냈다 — 레지스트리 기존 라벨의 이름을 화면에서 즉시 바꾸면 그 Topic을 쓰는 다른 모든 Digest의 표시가 같이 바뀌는데, 이건 이 리뷰 화면 하나의 스코프를 벗어나는 조작이라 컨센서스 위반으로 판단됐다(PR #506). 지금은 기존 라벨에 이름 수정 진입점 자체가 없다 — 검색 목록에서 첨부·해제만 가능하고, 미트볼(⋯) 메뉴는 신규 라벨에만 뜬다(`TopicSearchList.tsx`).
...
- **범위 참고 (2026-07-27, PR #506·#510 갱신)**: Topic과 같은 이유로 이름 수정 진입점 자체가 없다(`TagSearchList.tsx`) — 위 "기존 Topic은 이름 수정 불가" 참고.
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-28, PR #517)**: Tag 색상 필드(`tags.color`)가 생겼고, 리뷰 화면의 일관된 원칙(신규는 자유 편집, 기존은 첨부/제거만) 그대로 적용됐다 — 기존 Tag를 재사용해 붙일 때는 그 Tag가 이미 가진 색이 그대로 실려 오고, 여기서 바꾸는 진입점은 없다("Digest 리뷰 화면에서 Tag 추가 — 기존 선택" 참고).
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-28, PR #510·#515·#517 갱신)**: Topic과 같은 리팩터(`TagDraftRenameForm`) — 위 "신규 Topic 이름 수정 가능" 참고.

#### 신규 라벨 삭제

- **Given**: 유저가 Digest 리뷰 화면에서 이 리뷰 안에 신규로 만들어진(아직 확정 전) Topic·Tag를 보고 있다.
- **When**: 검색 목록에서 그 라벨 행의 미트볼(⋯) 메뉴를 눌러 편집 팝오버를 열고 삭제를 누른다.
- **Then**: 그 라벨이 리뷰 팔레트에서 완전히 사라지고, 붙어 있던 모든 Digest 후보에서도 함께 떨어진다. 확인창은 없다. 레지스트리 기존 라벨에는 이 진입점 자체가 없다(붙이기·떼기만 가능하다).
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-08-05, PR #551)**: 이름·색상을 고치는 그 편집 팝오버 안에 삭제를 함께 둔다 — 라벨 자체를 다루는 자리가 이미 거기라, 붙어 있든 아니든 한 번에 지울 수 있다. 삭제는 라벨 5개 상한과 무관하게 항상 가능해야 하므로, 상한에 닿아 검색·생성이 막힌 상태에서도 이미 붙은 신규 라벨 행은 계속 보여준다(붙은 라벨이 전부 레지스트리 기존이면 목록 자체를 안 그린다). 팝오버는 닫힐 때 이름을 커밋하는 구조라, 삭제로 닫힐 때는 그 커밋을 건너뛴다 — 안 그러면 리듀서에선 no-op이어도 실행취소 스택에 빈 칸이 하나 더 쌓여 삭제 한 번을 되돌리는 데 실행취소를 두 번 눌러야 한다.
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-15 / 2026-07-28 갱신)**: Topic은 Tag와 다르게 별도 필드 없이 이름만으로 즉시 생성됨. 신규 생성 뒤에도 이름을 계속 고칠 수 있다("신규 Topic 이름 수정 가능" 참고).
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-15 / 2026-07-28 PR #517 갱신)**: Tag가 Topic과 다르게 description 필수라 미니 폼을 거치는 구조. 신규 생성 뒤에도 이름·색상을 계속 고칠 수 있다("신규 Tag 이름 수정 가능" 참고).
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-28, PR #510)**: `DigestTopicPicker`/`DigestTagPicker` 둘 다 `registryId === null`(신규) 여부만 비교하는 `stable sort`로 신규를 앞으로 옮긴다 — "+" 표식의 의도("아직 확정 전이니 먼저 보고 판단하라")와 맞추기 위함. `id` 기반 식별(PR #515)이 전제된 뒤에야 정렬해도 삭제·수정이 엉뚱한 항목을 안 건드리는 게 보장된다. 구분자 스킵 로직(`showSeparator`)은 `DigestTopicPicker`에만 있다 — `DigestTagPicker`는 라벨을 Badge 칩으로 렌더링해 애초에 구분자를 쓰지 않는다.

#### 라벨 정렬 — 검색 목록

- **Given**: 유저가 Digest 리뷰 화면에서 Topic·Tag 추가 팝오버를 열고 검색 목록을 보고 있다.
- **When**: 이 후보에 이미 붙은 라벨과 안 붙은 라벨, 신규 라벨과 기존 라벨이 섞여 있다.
- **Then**: 이미 붙은 것이 항상 위에 오고, 그 안에서 신규가 기존보다 앞선다 — 붙은 신규 → 붙은 기존 → 안 붙은 신규 → 안 붙은 기존 순이다. 같은 그룹 안에서는 원래 순서가 유지된다.
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-08-05, PR #551)**: 위 "라벨 정렬 — 신규 먼저"와는 다른 표면이다 — 그쪽은 후보 카드에 붙은 칩 목록, 이쪽은 추가 팝오버 안의 검색 목록이다. 이 목록은 신규(draft) 항목과 레지스트리 후보를 따로 그리고 있어 정렬이 두 그룹을 못 넘었는데, 한 배열로 합친 뒤 `stable sort`로 "붙음"을 1차·"신규"를 2차 기준으로 정렬한다. "붙음"을 앞세우는 이유는 이 팝오버가 지금 이 후보의 라벨을 고치는 자리라, 현재 붙어 있는 것이 한데 모여 있어야 눈으로 확인하기 쉽기 때문이다.
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-15, PR #416)**: `DigestCandidateCard`의 타입 Select `onValueChange`가 `onBodyChange({ type })`(이전 타입 필드 없이 새 타입만)를 호출 — 컨펌 모달 없이 즉시 전환되는 스펙 그대로 구현. `DigestBodySchema`가 타입별 필드를 전부 optional로 둬 스키마 레벨 안전망은 있으나, 이 리셋 생성 자체(타입 Select 핸들러)를 직접 검증하는 테스트는 아직 없음(`confirmReviewFlow.test.ts`는 override 값이 주어졌을 때 잘 흘러가는지만 검증) — 멀티 에이전트 리뷰에서 지적됐고 이번 라운드엔 미반영.
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-14, PR #412)**: 구현됨 — `confirmDisabledReason`(순수 함수로 추출, 유닛 테스트 있음)이 `hasCandidates`를 최우선으로 판정. 버리기 액션은 이 비활성화와 무관하게 항상 사용 가능.
...
- **관여 화면**: Digest 리뷰 화면
- **범위 참고 (2026-07-14, PR #412)**: 구현됨 — "제목"은 Changeset 제목이 아니라 Digest 후보 자체의 제목(`DigestCandidateCard`의 `Input`, `Changeset.title` 컬럼은 이번 PR 스코프 밖). `confirmDisabledReason`이 `hasEmptyTitle`을 판정.

## 실행취소

> **이번 범위 밖.** Digest 리뷰 화면에서 확정 전 편집을 되감는 기능이라, 위 섹션과 함께 도달 경로가 닫힌다. 판정 화면에는 원래 없던 기능이다(확정 전 편집이라는 단계 자체가 없다).
...
- **범위 참고 (2026-07-28, PR #509)**: `ReviewDraftProvider`가 액션별 역연산표 대신 매 `dispatch` 직전 draft 전체 스냅샷을 `undoStack`에 push하고, `undo()`가 그걸 통째로 pop해 쿼리 캐시에 다시 써넣는 방식이라 "예외 없이 모든 액션 타입이 자동으로 실행취소 대상이 된다"(코드 주석). 스택은 세션 동안 GC되지 않는 캐시(`gcTime: Infinity`)에 맞춰 `MAX_UNDO_STACK_SIZE = 50`으로 캡을 둔다. 복원 시 `draftVersion`만은 스냅샷 값이 아니라 캐시의 현재 값을 유지해, 되돌리기 자체가 자동저장 버전 충돌(NM012)을 유발하지 않게 막는다. 단축키는 `mod+z`(전역), 제목·설명 등 `<textarea>` 안에서는 `enableOnFormTags: false`로 꺼져 있어 브라우저 네이티브 텍스트 실행취소에 양보한다.
...
- **범위 참고 (2026-07-28, PR #509)**: `redo()`도 동일하게 스냅샷 통째 push/pop 방식이며 같은 50개 캡(`MAX_UNDO_STACK_SIZE`)을 공유한다. 실행취소 이후 새 액션을 실행하면(되돌리기가 아닌 편집) `redoStack`이 비워진다 — 분기된 실행취소 이후 히스토리는 보존하지 않는다. 단축키는 `mod+shift+z`, "실행취소" 케이스와 동일하게 폼 필드 안에서는 꺼진다.
...
- **범위 참고 (2026-07-28, PR #509)**: "유지되는 것"과 "사라지는 것"이 서로 다른 저장소다 — 실행취소/다시 실행 스택은 `ReviewDraftProvider`의 React state라 새로고침하면 그냥 없어지지만, 편집 내용 자체는 이것과 별개로 1초 디바운스(`AUTOSAVE_DEBOUNCE_MS`)마다 서버로 자동저장되어 있어서 새로고침해도 안 사라진다 — 새로고침 시 화면은 그 서버 상태를 다시 fetch해 보여줄 뿐이다.
...
- **관여 화면**: 변경셋
- **범위 참고 (2026-07-29, 관계 판정 changeset 제목 생성 확장 슬라이스)**: 지금까지 title이 null이었던 걸 채웠다 — `apply_relation_changesets`(마이그레이션 `20260729150000_relation_conflict_title_and_batch_title.sql`)가 배치 changeset 생성 시점에 `sources.title`을 읽어 그대로 저장한다.
- **범위 참고 (2026-07-28, PR #514)**: `changeset-detail-service.ts`가 supports/replaces/resolves 셋을 전부 `unsupported`로 뭉개던 걸 `relation_confident_applied`로 분리해, Changeset 상세를 열면 관계 종류 캡션+끝점 Digest 카드가 실제로 보인다. 한 changeset에 확신 관계가 여러 건 담길 수 있어(배치당 changeset 1개, 성공한 관계마다 change 행 하나) 본문이 `relations` 배열이다.
...
> **이번 범위 밖.** 명세서 스스로 "정확한 매칭 조건은 미정"이라고 적어둔 상태라, 미정인 걸 이번에 만들 이유가 없다. Reference 상세의 핵심은 "이 Reference를 언급한 판단 목록"이고 그건 browsing-flow에 있다.
...
- **범위 참고 (2026-07-29, 관계 판정 changeset 제목 생성 확장 슬라이스)**: 관계 엔진 2단계 판정 콜(`worker.ts` `callJudgment`, `RelationJudgmentSchema`)이 conflicts 판정일 때 `conflictTitle`도 함께 뽑도록 확장되고, 그 값이 있으면 `apply_relation_changesets`(마이그레이션 `20260729150000_relation_conflict_title_and_batch_title.sql`)가 title로 쓴다. 없으면 기존 "A vs B"로 낮아진다 — duplicates의 `merge_draft.title` 폴백 패턴과 동일.
...
> **이번 범위 밖.** 컴플라이언스 삭제 요구가 아직 없고, "다른 원문 소속 relation changeset에만 남은 스냅샷"이라는 아주 좁은 경우에 비해 구현이 무겁다. 완전 삭제를 실제로 요구받을 때 다시 본다.
...
> **이번 범위 밖 — 섹션 전체.** 확정 후 내용 편집을 다음 라운드로 미뤘으므로 편집·수정 이력·편집 잠금·동시성 충돌은 모두 발생하지 않는다. **"아카이브 되살리기"만 살아남아 [`browsing-flow.md`](browsing-flow.md)로 옮겼다** — 아카이브는 이번에도 있고, 되살리기는 그 자리에서 manual changeset을 되돌리는 것이라 Digest·Reference 자신의 상세 화면 몫이다(2026-07-31 결정 로그의 "Manual 편집은 browsing-flow 몫" 정정이 여기서 실행됐다).
