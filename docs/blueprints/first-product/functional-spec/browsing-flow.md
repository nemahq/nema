...
- **When**: 변경셋 탭으로 전환한다.
- **Then**: 대기 중 개수 배지 없이, 변경셋 탭 안에 빈 상태 안내 문구가 표시된다.
- **관여 화면**: Space 오버뷰
...
- **When**: 카드 하나를 클릭한다.
- **Then**: 그 Digest의 상세가 우측 사이드뷰에 열리고, 원래 보던 스레드 피드는 그대로 유지된다(화면 전환 없음).
...
- **Then**: 새 탭이 만들어지지 않고, 이미 열려 있던 그 탭으로 전환된다.
...
- **Then**: 그 탭만 닫히고, 나머지 탭과 사이드뷰는 그대로 유지된다.
- **관여 화면**: Digest 상세, Reference 상세
...
- **Then**: 그 Digest의 상세가 같은 사이드뷰 안에 새 탭으로 열리고, 원래 보던 탭은 그대로 남아 있다.
- **관여 화면**: Digest 상세
...
- **When**: 그 멘션을 클릭한다.
- **Then**: 그 Reference의 상세가 같은 사이드뷰 안에 새 탭으로 열리고, 원래 보던 탭은 그대로 남아 있다.
- **관여 화면**: Digest 상세, Reference 상세
...
- **Then**: 그 Topic이 즉시 추가되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 Topic이 즉시 제거되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 Tag가 즉시 추가되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **When**: 그중 하나의 제거 액션을 실행한다.
- **Then**: 그 Tag가 즉시 제거되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 Digest가 관련 Digest로 즉시 연결되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **When**: 그중 하나의 제거 액션을 실행한다.
- **Then**: 그 관련 Digest 연결이 즉시 제거되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 링크가 즉시 추가되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 링크가 수정한 URL로 즉시 반영되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 외부 링크가 즉시 삭제되고, 별도 changeset은 생성되지 않는다.
- **관여 화면**: Digest 상세
...
- **Then**: 기존 Digest는 archive되고 새 Digest가 그 내용으로 생성되며, manual changeset이 즉시 closed+applied 상태로 기록된다. 보고 있던 탭은 새 Digest로 전환되어 계속 그 내용을 보여준다.
- **관여 화면**: Digest 상세
...
- **Then**: 로컬 편집 내용만 사라지고 서버엔 아무 변경도 남지 않는다. changeset은 생성되지 않고 원래 Digest가 그대로 유지된다.
- **관여 화면**: Digest 상세
...
- **Then**: 컨펌 모달이 뜨고, 확인하면 기존 타입 전용 필드 내용이 초기화된 채 새 타입의 필드로 전환된다.
- **관여 화면**: Digest 상세
...
- **Then**: 그 멘션이 편집 중인 본문에 삽입된다. 아직 changeset은 생기지 않고, 제출해야 이 Digest의 참조로 확정된다.
- **관여 화면**: Digest 상세
...
- **Given**: 유저가 Digest 상세를 평소 열람 중이다.
- **When**: 원문 보기 액션을 실행한다.
- **Then**: 그 Digest의 원본(Source) 탭이 같은 사이드뷰 안에 열리고(이미 열려 있으면 그 탭으로 포커스 이동), 이 Digest가 나온 위치가 하이라이트되어 표시된다.
- **관여 화면**: Digest 상세
