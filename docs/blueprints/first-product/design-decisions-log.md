...
**초안 탭 "정리하기" 재제출 버튼 — `intake.remember`로 컴포저 제출 버튼과 공용 키화**: needs-attention 초안의 재시도 버튼이 "정리"(AI 작업의 상태 동사)를 사람이 누르는 액션 라벨 자리에 쓰고 있어서, 클릭 후 버튼 자신의 pending 라벨("정리 중...")과 뒤이은 실제 처리중 인디케이터("정리 중")까지 겹쳐 "정리"가 세 번 보였다. 컴포저의 최초 제출 버튼("기억하기"/`intake.compose_submit`)과 뜻이 동일해(둘 다 "콘텐츠를 파이프라인에 밀어넣는 사람의 행동") 새 키를 만드는 대신 `compose_submit`을 `intake.remember`로 승격해 두 곳이 공유하게 했다 — 사람 행동(기억하기)과 AI 상태(정리 중)를 항상 분리해서 쓴다는 원칙을 이 버튼도 따르게 됨.
