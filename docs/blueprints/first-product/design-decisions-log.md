...
- 새 컴포넌트/토큰 없음. `SignInPage.tsx`의 기존 에러 상태(`error`)와 `role="alert"` 표시 영역을 그대로 재사용 — Supabase가 리다이렉트에 `#error=...` 해시로 실어 보내는 에러를 마운트 시 파싱해 같은 `error` state에 꽂아 넣는 방식으로 처리(별도 UI 분기 없음).
- 번역키 `auth.magic_link_invalid` 추가(en/ko 라이팅 모두 작성, `docs/guides/ux-writing.md` 톤 적용).
