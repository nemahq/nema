...
**회귀 확인 — 기존 `{count}` 단순 보간 키 8개 전부**: `overflow_count`, `retrieval_doc_count`, `drafts_delete_waiting_confirm_title`, `drafts_delete_waiting_partial_failure`, `delete_pending_drafts_label`, `draft_organizing_elapsed_seconds`, `delete_blocked_description`, `digest_section_title`/`reference_section_title`(원래 값). 브라우저에서 실제 앱으로 두 가지 방식으로 확인: (1) 초안 전체 삭제 다이얼로그를 한/영 양쪽에서 실제로 열어 `drafts_delete_waiting_confirm_title`이 "초안 11개를 모두 삭제할까요?" / "Delete 11 drafts needing attention?"로 정상 렌더링되는 것, Digest 리뷰 화면에서 `digest_section_title`이 "다이제스트 (1)" / "Digest (1)"로 렌더링되는 것을 스크린샷으로 직접 확인. (2) 나머지는 개발 서버가 서빙 중인 실제 `tolgee` 싱글턴을 브라우저 콘솔에서 동적 import해 `tolgee.t({key, params})`로 8개 키 전부 한/영 양쪽 직접 호출 — 전부 기존과 동일하게 렌더링됨을 확인. 회귀 없음.

**버그 발견 및 수정 — ICU `#`는 plural 블록 밖에서 무효**: `effect_digest`/`effect_reference` 등을 처음엔 `"{count, plural, one {digest} other {digests}} #"`(라벨은 plural 블록 안, 카운트는 블록 밖에 `#`로) 형태로 작성했는데, 브라우저에서 실제 `tolgee.t()` 호출로 확인하니 `#`이 블록 밖에서는 리터럴 "#" 문자로 그대로 출력됐다(예: "digest #"). ICU MessageFormat 표준상 `#`는 plural/selectordinal 블록 **안에서만** 카운트로 치환되는 특수 토큰이라 블록 밖에선 그냥 글자다. `"{count, plural, one {digest #} other {digests #}}"`처럼 `#`를 블록 안으로 옮겨 수정 — 라벨과 카운트를 한 문자열에 압축하려는 리팩터에서 반복될 수 있는 함정이라 남긴다. **amendment(PR 작성 시 UX 라이팅 게이트 반영)**: 이 순서(라벨 → 카운트, "digest 1")는 원래 JS가 `${label} ${count}`로 이어붙이던 순서를 그대로 보존한 것이었는데, UX 라이팅 체크가 영어 관용 표현상 카운트가 먼저 오는 쪽("1 digest")이 자연스럽다고 지적해 최종적으로 `"{count, plural, one {# digest} other {# digests}}"`(카운트 → 라벨)로 한 번 더 바꿨다. `digest_section_title`류의 `"Digest (1)"`(괄호 카운트)는 순서 상관없이 관용적이라 그대로 유지, `effect_*` 5개만 순서를 바꿨다.

**전환 범위 — 요청받은 4곳 중 2곳(`digest_section_title`/`reference_section_title`)은 그대로, 나머지 2곳(`effect_digest`/`effect_reference`)은 같은 루프의 5개 키 전부로 확대**: `ChangesetListRow`의 effect 요약(`summarizeChangesetEffect`, `utils.ts`)은 `statement`/`relation`/`source`/`digest`/`reference` 5개 타입을 같은 `Object.entries().map()` 루프에서 처리한다. 이전엔 각 타입 라벨(`t(key)`, 카운트 없는 단어)에 JS가 `${label} ${count}`로 카운트를 직접 붙였다. `effect_digest`/`effect_reference`만 번역 문자열 안에 카운트를 흡수시키고 나머지 3개는 그대로 두면, 콜사이트가 타입마다 다르게 분기해야 하거나(지저분함) 5개 전부에 `t(key, {count})`를 균일하게 호출하는 순간 나머지 3개는 카운트가 통째로 사라진다(번역 문자열에 `{count}`가 없으므로) — 둘 다 나쁜 선택이라 5개 전부를 번역 문자열 안에 카운트를 흡수하는 동일한 패턴으로 맞췄다. `statement`/`relation`/`source`도 영어에서 자연스럽게 복수형이 있는 가산명사라 pluralize하는 데 추가 판단 비용이 들지 않았다. `summarizeChangesetEffect`/`changesetDisplayTitle`의 `t` 시그니처를 `(key, params?: {count: number}) => string`으로 넓히고 콜사이트는 `t(key, {count})` 한 줄로 단순화(기존 `${key ? t(key) : type} ${count}` 삼항 제거).

**한국어는 ICU plural 블록을 쓰지 않고 `{count}` 단순 보간 그대로 둠**: 한국어는 문법상 단/복수 구분이 없어 `one`/`other` 두 갈래를 만들어도 값이 항상 같다. 게다가 Intl.PluralRules의 `ko` locale은 CLDR상 `other` 카테고리만 존재해 `one` 분기 자체가 런타임에 절대 선택되지 않는 죽은 코드다 — `{count, plural, one {…} other {…}}`로 감싸는 대신 기존처럼 `"다이제스트 ({count})"`/`"다이제스트 {count}"` 그대로 두는 게 더 단순하고 오해의 소지도 없다. `digest_section_title`/`reference_section_title`의 ko 값은 이미 `{count}`를 쓰고 있어 무변경, `effect_*` 5개의 ko 값에는 원래 JS가 붙이던 카운트를 문자열 안으로 옮기기만 했다(`"다이제스트"` → `"다이제스트 {count}"`).

**전환 안 한 곳**: 위 회귀 확인 대상 8개 키(`overflow_count` 등)는 지침대로 스코프 밖 — 전부 "N개 더", "N건의 문서" 류로 카운트 뒤에 조사/단위가 붙는 한국어 UX 문구가 원본이고, 영문판도 대부분 가산명사가 아니거나("more", "documents"는 이미 복수형 고정) 복수형 분기가 의미를 더하지 않는 문구들이라 이번 스코프(digest/reference 섹션 타이틀 + effect 요약)와 성격이 다르다고 판단해 손대지 않았다. 필요해지면 개별 판단으로 후속 전환.

**검증**: `pnpm typecheck`/`lint`/`test`(apps/web) 전부 통과. 브라우저로 한/영 양쪽에서 Digest 리뷰 화면(`digest_section_title` count=1), 초안 삭제 다이얼로그(`drafts_delete_waiting_confirm_title` count=11) 직접 렌더링 확인 + 실행 중인 `tolgee` 싱글턴을 통해 위 회귀 대상 8개 키와 신규 전환 4개 키(+ `effect_relation`/`effect_source`/`effect_statement`) 전부를 다양한 count 값으로 직접 호출해 최종 문자열 확인. `effect_digest`/`effect_reference`가 실제 화면에 나타나는 케이스(공동 인용 등으로 `sourceTitle`이 없는 non-ingestion changeset)는 현재 시딩된 로컬 데이터에 없어 화면 스크린샷으로는 확인 못 했다 — 대신 위 `tolgee.t()` 직접 호출로 최종 렌더링 문자열을 확인했고, 이 콜사이트가 이미 라이브로 검증된 `digest_section_title`과 동일한 `t(key, {count})` 경로를 타므로 리스크는 낮다고 판단.
