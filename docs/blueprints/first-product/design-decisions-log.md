...
- **되살리기·원문도 삭제하기는 Changeset 상세 화면에만 둔다**: review-flow.md의 두 케이스 Given이 둘 다 "changeset 상세를 보고 있다"로 시작해, `mvp-wireframe.html`(Digest 리뷰 화면에도 되살리기 버튼이 나타나는 데모)보다 이 문서를 우선했다. Digest 리뷰 화면이 버려진 뒤에는 "Changeset 상세 보기" 버튼 하나만 남고, 되살리기는 거기서만 가능.
...
- **"원문 삭제 후 되살리기 비활성화"는 `listChangesets`의 `sourceStatus`로 미리 판정**: PM 확인 후 BE가 같은 세션에서 `listChangesets`에 `sourceStatus`(원문 Source 상태)를 추가로 얹어줘서, 화면 진입 시점부터 `entry.sourceStatus !== "pending"`이면 되살리기·원문도 삭제하기 버튼을 바로 비활성화한다. "원문도 삭제하기" 성공 직후처럼 invalidate 반영이 아직 안 끝난 짧은 순간은 `trashSource.isSuccess`로 같이 커버(`sourceTrashed = trashSource.isSuccess || entry.sourceStatus !== "pending"`) — 서버 가드(`restore_ingestion_review`의 `NM008`)는 그대로 최종 방어선.
...
- **"원문도 삭제하기"만 확인 다이얼로그(`DeleteSourceDialog`와 같은 weave `Dialog` 패턴), discard/restore/revert는 없음**: review-flow.md가 discard에 "컨펌 모달 없이" 즉시 반영을 명시한 것과 대칭으로, 되살리기·되돌리기도 같은 결의 즉시 반영 액션이라 모달을 안 뒀다. 원문 완전 삭제(`trash_source`)만 비가역성이 다른 급이라 기존 삭제 확인 패턴을 그대로 재사용.
...
- **`sourceTrashed` → 판정 로직·이름 재정비**: 실제로는 원문이 `active`(그 사이 다른 리뷰가 같은 원문을 재인제스천해 확정한 경우)여도 true가 되는데 이름이 "trashed"만 가리켜 오해 소지가 있었다. `restoreBlocked` + `blockReason`("trashed" | "reprocessed")으로 갈라, "원문이 삭제됨" 문구가 실제로는 안 지워지고 다른 리뷰로 처리된 경우까지 잘못 안내하던 것도 같이 고쳤다(`review.detail_source_reprocessed_disabled_reason` 신설).
...
### 2026-07-15 — Source 원문 편집 + 제목 생성을 디제스천에서 분리 (PR #415)

"결과없음(empty)"에서 재추출을 눌러봐야 원문이 그대로면 같은 결과가 나올 뿐이다 — 재추출이 의미를 가지려면 그 전에 원문(body)을 고칠 수 있어야 한다. 이 김에 제목 생성 방식도 다시 봤다: 제목은 Digest 추출 결과에 의존하지 않는데도 추출과 같은 무거운 LLM 콜(원문 전체 분석, standard 티어)에 얹혀 나오고 있었다.

- **제목 생성을 디제스천에서 완전히 분리**: body만 보는 별도 nano 콜로 떼어내 Source 생성 시점에 한 번 띄운다(ChatGPT 사이드바 제목이 응답 완료를 안 기다리고 첫 메시지만 보고 뽑히는 것과 같은 패턴). 응답을 안 붙잡는다(`fillSourceTitle`, fire-and-forget) — 제목 콜이 죽어도 원문 저장은 이미 끝난 일이고, 제목 없는 원문은 화면이 body 미리보기로 그린다. 재시도·큐도 없다 — 제목은 평생 한 번만 시도하는 값이다(`trackEvent`와 같은 부수효과 호출 규약). nano엔 body 전체(최대 100k자) 대신 앞 4,000자만 넣는다 — 도입부만 봐도 제목은 나오고, 장문 하나가 콜 비용·지연을 튀게 하는 손해가 크다.
...
- **`update_source_body` 신설 — 원문 편집 가드**: `update_source_title`과 같은 가드(pending + 처리 중 아님)에 "열린 pending 리뷰 없음"을 하나 더 얹는다. 리뷰에 뜬 Digest 후보들이 바로 그 body에서 뽑힌 것이라, 원문을 갈아치우면 더는 존재하지 않는 문장에서 나온 후보가 된다. 실제로 열리는 자리는 cancelled·failed·empty·discarded(리뷰를 사람이 버린 뒤) 넷 — 전부 "지금 화면에 걸린 후보가 없는" 자리다. body를 고쳐도 title은 그대로 둔다(비우면 사람이 정한 제목이 다음 생성 콜에 덮이는, 방금 없앤 문제가 되돌아온다 — 애초에 재추출도 title을 안 건드리므로 이 우려 자체가 구조적으로 사라졌다).
- **BE만 착지, FE는 후속**: `source.updateBody` tRPC 뮤테이션까지는 이번 슬라이스에 포함되지만 편집 액션 UI(버튼·다이얼로그)는 없다 — `intake-flow.md`의 "초안에서 Source 원문 편집" 케이스는 미체크로 남김.
...
**빈도 — 문턱·디바운스·쿨다운 전부 없음, 완료마다 즉시 알림**: 개수 문턱을 두면 "원문 하나만 던지고 자리를 비운" 시나리오(이 기능이 가장 필요한 경우)에서 알림이 아예 안 뜨는 모순이 생겨 기각. 인위적 지연(디바운스)은 "왜 안 오지" 불안을 새로 만들어 오히려 목적을 해침(Claude Code·Slack도 즉시 알림, 묶지 않음). Nema의 실제 볼륨(유저당 하루 한 자릿수)도 "폭풍" 방지가 필요할 만큼 높지 않음. 비정상 반복 생성은 알림 레이어가 아니라 트리거 원인에서 막을 것.
...
**`SpaceOverview`/`SpaceOverviewPage` props를 discriminated union으로**: `activeTab: "changesets"`일 때만 `subTab`/`onSubTabChange`가 필요해, optional prop 대신 유니온으로 타입 레벨에서 강제했다(topic 분기에서 subTab을 실수로 참조하면 컴파일 에러). 대신 `props.activeTab === "changesets"` 형태로만 좁혀지므로(구조분해한 로컬 변수로는 안 좁혀짐) 그 분기 안에서는 원문 `props` 객체를 그대로 참조.
...
**두 화면 공유 브레드크럼을 `ReviewNavigationBar`로 뽑음**: `OpenReviewScreen`엔 `NavigationBar`가 아예 없어(짝인 `ClosedReviewScreen`엔 있음) chrome이 어긋나 있었는데, 채워 넣으니 두 화면이 같은 `[Space] › 변경사항(open) › 제목` 3단 브레드크럼을 각자 인라인으로 짜는 중복이 생겼다 — 상태(open/closed)와 무관한 순수 표현이라 `ReviewNavigationBar`로 뽑아 둘 다 쓴다. 가운데 항목은 open/closed 모두 변경사항 탭 기본 진입(open 서브탭)으로 보내는 기존 동작을 유지했다. `NavigationBar`는 스크롤 컨테이너 밖 형제로 둬야 해(sticky 바운스 문제, 그 컴포넌트 주석 참고) `OpenReviewScreen`의 `main`을 스크롤 담당에서 떼고 `ReviewNavigationBar` + `data-main-scroll-area` 2단으로 재구성했다. **props는 `spacePublicId`/`title`만 받고 Space 이름은 컴포넌트가 직접 해석한다** — 이름은 publicId에서 `useSpaceListSuspenseQuery`(사이드바가 이미 캐시)로 파생되는 값이라, 두 호출부가 각자 해석해 `spaceName`을 넘기는 건 훅으로 직접 부를 수 있는데 굳이 prop으로 흘리는 꼴이었다(Kyle 지적). 그 결과 `OpenReviewContent`는 space 조회를 통째로 걷어냈고, `ClosedReviewScreen`도 Body에 내려주던 `spaceName` prop이 사라졌다.
...
**포커스 전환 — Digest 리뷰/Changeset 상세 화면 폴리싱 시작**: 지금까지 구현이 `docs/poc/mvp-wireframe.html`을 빠르게 옮긴 것이지 다듬어진 게 아니라는 전제로, Changeset 상세부터 화면을 사실상 빈 상태에서 재설계하기로 했다(기존 되돌리기/되살리기/원문삭제 로직·훅 파일 전부 제거, `knip`이 고아 코드로 잡아냄). 착수 전 정통 리뷰 UI 패턴(GitHub PR diff, Google Docs/Word 교정, Reddit 모더레이션 큐 등)과 AI-native 리뷰 트렌드(Cursor/Windsurf diff 승인, Linear Triage Intelligence, Devin 사전 플랜 승인 등)를 리서치해 참고 근거로 남겼다 — Linear의 "AI 제안값과 사람 편집값을 시각적으로 절대 안 섞는다"는 원칙이 Nema의 "판단 근거 추적 가능"이라는 최상위 기준과 가장 맞닿아 있어 후속 작업(콘텐츠 헤더·본문)에서 참고 대상.
...
### 2026-07-19~20 — Changeset 상세 헤더 완성: 상태 pill·되돌리기·URL 정리 (PR #437·#438)

**작성자/리뷰어 필드 — 헤더에서 보류**: 헤더 스펙(surface-inventory.md)이 "리뷰어/작성자"를 요구하지만, 실제로 `authorId`는 07-modeling.md상 "콘텐츠를 만든 주체"이지 "판단을 내린 사람"이 아니다 — ingestion(항상 사람이 확정/버림)과 판정형 relation은 `authorId`가 늘 null이라 지금 로직대로면 "Nema"로 잘못 표시되고, 확신 관계 자동 적용(진짜로 사람 개입 없음)만 정확하다. "판단자" 개념 자체가 스키마에 없는 진짜 데이터 갭이라 판단해, 단일 유저 단계에서 부정확한 값을 보여주느니 그냥 비워두기로 했다 — 정확히 채우려면 review RPC들이 호출자 id를 기록하는 스키마 확장이 필요(별도 백엔드 작업, 지금 스코프 아님).

**시각 필드 — `createdAt`이 아니라 `updatedAt`을 새로 노출**: Changeset 상세는 항상 closed 상태만 보여주는 화면이라 "언제 열렸나"보다 "언제 판단이 내려졌나"가 맞는 의미다. `listChangesets`엔 `updated_at`이 아예 없어서(`created_at`만) 서비스 select·반환 타입에 한 줄씩 추가해 노출했다. 이 값이 신뢰할 수 있는 이유: `revert_changeset`은 되돌려지는 원본 changeset row를 UPDATE하지 않고 새 changeset만 INSERT하므로(§4.4), 나중에 누가 되돌려도 이 값이 오염되지 않는다 — "그 판단이 최종 내려진 시각"만 정확히 담는다. `createdAt`/`updatedAt` 분기 로직(status별로 어느 쪽을 쓸지)은 이 화면 안에선 pending이 절대 안 나와 분기가 죽은 코드가 되므로 안 만들고, 실제로 필요해질 `ChangesetListRow`/Digest 리뷰 화면 차례에 만들기로 미뤘다.

**상태 pill — 리스트 행 아이콘 톤을 그대로 pill 전체로 확장**: `Badge`(weave, tint 배경+accent 텍스트)를 버리고 새 컴포넌트(`ChangesetStatusPill`)를 만들었다. 여러 라운드 조정 끝에 확정된 규칙: `filled`(applied/rejected)는 아이콘의 배경·텍스트 색을 pill 전체 배경/텍스트로 그대로 확장(아이콘 자체 원형 배경은 제거, 리스트 행과 달리 pill 배경이 이미 그 역할을 함). `outline`(pending)은 테두리·아이콘만 브랜드색(`text-brand`)을 유지하고 라벨 텍스트는 별도 톤(`text-fg-primary`, 처음엔 `fg-secondary`로 시도했다가 한 단계 올림)으로 분리했다 — 배경 없는 pill에서 라벨 전체가 브랜드색이면 링크처럼 보이거나 과도하게 강조돼 보이는 문제. `icon.kind`(outline/filled) 분기가 JSX 두 곳(컨테이너 className, 라벨 className)에 중복돼 있던 걸 `pillVisual()` 하나로 모았다 — kind가 늘거나 톤 규칙이 바뀌면 한 곳만 고치면 됨. pending을 filled(청록 채움)로 통일하는 안도 논의했으나, "outline=아직 진행 중(가벼움), filled=결론 남(무거움)"이라는 `changesetStatusIcon`의 기존 설계 의도(무게로 진행 상태를 신호)를 유지하기로 하고 보류했다 — 이 화면엔 pending이 안 뜨니 지금은 무해한 판단.

**되돌리기 버튼 — `neutral` variant, 아이콘 없음, 컨펌 모달 없음**: 톤은 `danger`(위험해 보이면 안 됨 — GitHub도 PR "Revert" 버튼을 빨간색으로 안 씀, 되돌리기는 Nema의 핵심 신뢰 구조가 보장하는 정상 행위)도 `primary`(이 화면의 주된 행동처럼 과대 대표되면 안 됨 — 대부분의 방문은 "확인"이 목적이지 되돌리기가 아님)도 아닌 `neutral`로 확정. 컨펌 다이얼로그는 초안엔 있었으나 제거했다 — "되돌리기 자체도 다시 되돌릴 수 있다"는 구조적 안전망이 이미 있는데 절차적 확인까지 두는 건 "구조적으로 항상 되돌릴 수 있다는 확신"이라는 제품 철학과 어긋난다는 판단(GitHub Revert 버튼도 확인 모달 없이 즉시 실행). 클릭 → 즉시 뮤테이션 → 성공 시 새 revert changeset으로 자동 이동, 중복 클릭만 `disabled`로 방지.

**용어 — "되돌리기"/"Revert" 확정 (웹서치 근거)**: 위키백과 용어집이 "되돌리기(revert, 이전 판으로 복구)"·"일괄 되돌리기(rollback)"·"편집 취소(undo, 특정 판 diff만 되돌림)"를 명확히 구분한다는 걸 확인 — Nema의 "되돌리기"(적용된 changeset을 새 changeset으로 원상복구)는 정확히 위키백과의 revert 개념과 대응하고, Nema의 "실행취소"(Digest 리뷰 화면에서 확정 전 로컬 편집을 되돌리는 것, review-flow.md 별도 섹션)는 오히려 위키백과의 undo 쪽에 가깝다 — 두 액션을 다른 이름으로 부르고 있던 게 우연이 아니라 정확한 구분이었음을 확인. 영문 "Revert"는 GitHub PR의 "Revert" 버튼(머지된 PR을 새 PR로 원상복구)이 정확한 메커니즘 선례.

**Changeset 단건 조회 + URL number 전환 (PR #438, 백엔드+프론트 세션에 위임)**: Changeset 상세 본문(타입×outcome 스냅샷)을 만들려는 시점에 두 가지 근본 문제를 발견 — (1) 전용 단건 조회가 없어 `listChangesets`(Space당 최근 100건 캐시)를 id로 스캔하는 방식이라 100건 밖 오래된 changeset을 못 찾음, (2) `listChangesets`는 애초에 `changes.data`(실제 변경 콘텐츠)를 안 내려줘서 본문에 보여줄 콘텐츠 자체가 없음(pending 리뷰 전용 `digestReview.getReview`만 콘텐츠를 반환하는 유일한 경로였음). 3개 아이템으로 정리해 위임: ① `changeset.getByNumber({spaceId, number})` 신설(surface-inventory.md 7케이스 커버, `changes.data` 파싱 대신 `target_id`로 실제 테이블 join — archive는 상태 전환일 뿐이라 join 결과 자체가 그 시점 스냅샷과 동일) ② URL을 UUID `$changesetId`에서 `$changesetNumber`로 전환(Space는 opaque publicId를 새로 만들었지만 changeset은 이미 `(space_id, number)` 유니크인 사람이 읽는 식별자가 있어 새로 안 만듦) ③ `listChangesets`는 메타데이터 전용 유지 원칙 명문화. PR 작업 중 스펙 문서(surface-inventory.md·review-flow.md)와 실제 백엔드 상태의 괴리도 발견됨 — 관계 판정(충돌 패자 archive, 중복 Digest 병합)이 실제 RPC엔 없고 판정 모드 UI 자체가 아직 어디에도 구현 안 됨(product-decisions-log #18에 기록, `getByNumber`는 지어내지 않고 지금 실제로 있는 것만 반환).

**URL 스킴 정리 제안 (미구현, 옆 세션에 전달)**: v2 표면 URL을 훑어보니 같은 changeset 개념이 `/changes`(목록)·`/changesets/$number`(닫힌 상세)·`/review/$changesetId`(열린 상세) 세 가지 다른 명사로 갈려 있었다. `changes`(diff 행 테이블)와 `changesets`(리뷰 단위 테이블)는 코드에서 이미 다른 엔티티를 가리키는 이름이라, 목록 URL이 `changes`를 쓰면서 실제로는 changesets 목록을 보여주는 건 진짜 이름 불일치다. 제안: 목록은 `/changesets`로 통일, 상세는 open/closed 구분 없이 `/changesets/$number` 하나로 합치고(`/review` suffix 제거) 라우트가 상태를 보고 Open/Closed 어느 화면을 렌더할지 정하는 게이트만 추가 — GitHub PR이 정확히 이 패턴(`/pull/123`이 open이든 merged든 URL 안 바뀜)이라 그대로 참고. `space`(단수)+id는 그대로 둠(`/spaces` 목록 페이지 자체가 없어 스코프 프리픽스로는 자연스러움). 마침 옆 세션이 open 쪽 changesetId→changesetNumber 전환을 진행 중이라 같은 자리에서 흡수하도록 전달했다 — 기존 링크·북마크는 없음(pre-PMF, 스텔스 모드)이라 부담 없는 변경.

---

### 2026-07-19 — Changeset 관련 URL 두 가지 정리: `/changes` → `/changesets`, Open/Closed 상세 URL 통합

**목록 URL을 `/space/$id/changes` → `/space/$id/changesets`로**: 코드에서 `changes`(diff 행 테이블)와 `changesets`(리뷰 단위 테이블)는 이미 서로 다른 엔티티를 가리키는데, 목록 URL만 `changes`를 쓰면서 실제로는 changesets 목록을 보여주고 있었다 — 이름이 가리키는 대상과 실제 콘텐츠가 어긋난 상태였다. `?subTab=open|closed` 쿼리는 그대로 유지, 라우트 경로 문자열만 교체(`ReviewNavigationBar`·`SpaceOverview`의 링크도 함께).

**Open 리뷰(`/review/$number`)와 Closed 상세(`/changesets/$number`) 두 라우트를 하나(`/changesets/$number`)로 통합**: GitHub의 PR 페이지(`/pull/123`이 merge 여부와 무관하게 URL이 안 바뀌고, 상태에 따라 같은 페이지가 다르게 렌더되는 것)를 그대로 참고했다. `changeset.getByNumber`로 status를 먼저 읽어 pending이면 `OpenReviewScreen`, 아니면 `ClosedReviewScreen`을 그리는 게이트(`ChangesetDetailScreen`) 하나만 라우트 레이어에 추가 — 두 화면 컴포넌트 자체는 합치지 않았다(편집 가능한 초안 vs 확정된 읽기전용 기록이라는 성격 차이가 여전히 유효해서, 억지로 하나로 묶으면 조건 분기만 늘어난다).

**공급자(백엔드 계약)가 기준, 소비자가 맞춘다**: 이 작업 전에 이미 `changeset.getByNumber`(변경사항 상세)는 number 기준으로 정리돼 있었지만 `digestReview.get`(Open 리뷰)은 UUID(`changesetId`) 그대로였다. 처음엔 dev-harness가 UUID를 이미 들고 있다는 이유로 `digestReview.get` 입력을 `{changesetId} | {spaceId, number}` 유니온으로 두 다 받게 하려 했으나, "공급하는 쪽이 소비하는 쪽에 맞추는" 방향이 거꾸로라는 지적으로 뒤집었다 — `digestReview.get` 입력을 `{spaceId, number}` 하나로 확정하고, dev-harness(`DigestReviewCard`·`useDigestReviewQuery`·`useUpdateReview`)와 MCP 툴 설명(`get_ingestion_review`·`list_pending_sources`)이 그 계약을 따라오게 했다. `listPendingSources`에 `reviewChangesetNumber`를 추가해(기존 `reviewChangesetId`는 유지, 상태 판정용 존재-체크는 계속 그 필드로) dev-harness가 number로 진입할 수 있게 했다.
...
### 2026-07-21 — Reference 목록/상세 첫 구현 (browsing-flow 6케이스, 목록·상세·직접수정·Tag추가·Tag제거·아카이브)

BE가 같은 워크트리에서 동시에 `reference.get`/`update`/`archive`/`addTag`/`removeTag`를 새로 얹는 중이었고, 착수 시점엔 `list`/`citingDigests`/`trash`만 있었다(목업으로 시작하려던 계획). 작업 도중 BE 쪽 계약이 실제로 착지해서, 목업 레이어를 걷어내고 바로 실 tRPC 훅으로 교체했다 — `apps/web/src/features/reference/hooks/*`가 트리 전체를 담당.

**공용 라벨 칩 UI를 `components/ui/`·`hooks/`·`utils/`로 승격**: `TagAddPopover`/`LabelChipShell`/`RegisteredLabelChip`(컴포넌트), `useTagListSuspenseQuery`(훅), `labelSearch.ts`(순수 함수)가 지금까지 `features/review/` 전용이었는데 이번에 Reference 상세도 같은 태그 추가 팝오버가 필요해져 2곳 이상 소비 규칙(`apps/web/CLAUDE.md`)에 걸렸다. `DraftLabelChip`(초안 상태 개념이 있는 칩)은 review에 남기고, 상태 없는 순수 UI만 옮겼다 — 이동만, 로직 변경 없음.

**용어 — 컬렉션은 "위키"/"Wiki", 개별 항목은 "레퍼런스"/"Reference" 유지**: LNB·화면 타이틀은 기존 결정(`workspace.references`="위키"/"Wiki", 이 로그 상단 참고)을 그대로 재사용했다. 다만 review-flow의 기존 실제 카피(`reference_type_label`="레퍼런스 유형", `reference_remove_action`="이 레퍼런스 후보 삭제" 등)를 다시 확인해보니 **개별 엔티티를 가리킬 땐 "레퍼런스"를 그대로 쓰고 있었다** — "위키" 결정은 컬렉션 이름(LNB 항목·화면 제목)에만 적용되고, "이 레퍼런스를 아카이브할까요?" 같은 개별 항목 문구는 기존 관례대로 "레퍼런스"를 유지하는 것이 일관적이다. 새 `reference.*` i18n 키 전부 이 구분을 따랐다(en/ko 실작성).

**목록 컬럼 — surface-inventory.md 설계(타입·이름·설명·태그·인용·최종수정)와 실제 BE 계약이 어긋남, lean-list로 조정**: `reference.list`는 `{id,type,title,status,createdAt}`만 내려준다(설명·태그·인용수·최종수정은 없음) — `reference.get`/`citingDigests`는 단건 조회 전용. 이건 Changeset 목록의 기존 결정(#18 "listChangesets는 메타데이터 전용 유지")과 같은 lean-list 패턴이라, 목록 컬럼을 실제 계약에 맞춰 **타입·이름·상태·생성일**로 줄였다(원래 설계의 설명·태그·인용·최종수정은 상세 패널에서만 보여줌). 태그 필터도 목록 레벨에서 뺐다(목록 응답에 태그가 없어 필터 후보를 만들 재료가 없음). **PM 확인 필요**: surface-inventory.md의 원래 컬럼 설계를 그대로 살리려면 BE가 `list`를 풍부하게 만들어야 하고, 아니면 이 lean-list 조정을 문서에 반영해야 한다 — 이번 세션은 실제 계약을 기준으로 구현을 맞췄을 뿐 문서는 아직 안 고쳤다.

**직접 수정 — body만이 아니라 title도 편집 가능**: 처음엔 wireframe의 `reference-red-edit` 마크업(설명 textarea만, 제목·타입은 읽기 전용)과 product-decisions-log #16(병합 편집은 body만)을 근거로 body만 편집하게 만들었다가, browsing-flow.md의 실제 케이스 문구("수정 액션을 실행해 **이름·설명 등**을 고치고")를 다시 읽고 title도 편집 필드로 추가했다 — #16은 리뷰 화면의 "기존 Reference 병합 편집"(다른 케이스)에 대한 결정이라 이 케이스엔 적용되지 않는다. type은 그대로 읽기 전용(식별자 성격이 강해 편집 대상에서 뺌, PM 확인 없이 내 판단). `update_reference` RPC가 전체 상태(type·title·body·externalUrls)를 받는 계약이라, 편집 폼이 안 건드리는 type·externalUrls도 현재 값 그대로 실어 보낸다(생략하면 빈 값으로 덮어써지는 트랩, 서비스 코드 주석 참고) — 외부 링크 CRUD 자체는 이번 슬라이스 범위 밖(browsing-flow.md의 별도 케이스, 6케이스에 없음).

**아카이브 — 되살리기 버튼 없음**: BE에 archive(active→archived)만 있고 restore/unarchive RPC가 아직 없다. `ReferenceArchivedBanner`는 상태 배지만 보여주고 액션은 없다 — 제품결정 #12("Reference 목록에 상태 필터로 발견+복구")의 "복구"가 아직 구체적인 되돌리기 동작으로 이어지지 않은 상태라는 뜻. 목록에 상태 필터(활성/아카이브됨/전체, 기본값 활성)는 넣어서 발견은 가능하게 해뒀다.

**사이드뷰 — 멀티탭 인프라 새로 안 만듦**: `ChangesetSidePanelProvider`(review 전용 다중탭 셸)를 재사용하는 대신 `components/ui/SidePanel`(범용 리사이즈 셸)만 직접 써서 단일 패널로 구현했다(지침 그대로). 같은 id 재클릭 시 탭 전환 같은 동작은 애초에 탭이 하나뿐이라 필요 없다.

**로컬 실행 검증 — 브라우저 대신 실 서버·DB 직접 호출**: claude-in-chrome은 사용자가 켜달라고 할 때만 쓴다는 기존 방침이 있어(피드백 메모), 이번엔 브라우저 대신 로컬 서버(포트 3011, BE의 uncommitted 변경 포함)에 curl로 직접 tRPC를 호출해 6케이스를 실측했다 — 관리자 API로 테스트 유저·워크스페이스·Space·Reference·Tag를 직접 시딩(로컬 전용, 검증 후 전부 삭제)하고 `reference.list`(빈 상태 포함)·`get`·`update`(title+body 반영·updatedAt 갱신 확인)·`addTag`/`removeTag`(태그 배열 변화 확인)·`archive`(상태 전이+목록 반영 확인)를 전부 실제 응답으로 확인했다. 이 과정에서 로컬 공유 Supabase의 `workspace_members`/`workspaces`/`profiles` 테이블에 `authenticated` role SELECT grant가 아예 빠져 있어(왜 빠졌는지는 미확인) `workspace.bootstrap`이 로컬에서 항상 실패한다는 걸 발견 — 임시로 두 테이블에 SELECT를 걸어 진단만 하고 즉시 되돌렸다(공유 DB라 영구 변경은 안 함). 이건 이번 슬라이스와 무관한 로컬 인프라 갭이라 코드로 고치지 않고 여기 기록만 남긴다 — 다음에 로컬 풀스택으로 뭔가 검증하려는 세션은 같은 벽에 부딪힐 것이다.

---

### 2026-07-21 — Digest 카드 헤더·바탕 재설계 진행 중 (인수인계 계속, wt-3 `polish/changeset-detail-body`)

이전 인수인계(바로 위 섹션) 이후 진행분. **기존 구현(다이제스트 카드 시각 위계 1라운드 포함)은 "기능만 보기 위한 임시 구현"으로 취급하고, UI·UX를 사실상 처음부터 다시 잡는다는 전제로 진행했다** — 그래서 아래는 이전 섹션의 규칙을 갱신하는 게 아니라, 더 근본적인 층위(이 카드가 뭐여야 하는가)부터 다시 내려온 결과다.

**최상위 맥락**
- **핵심 한 문장**: "이 카드는 AI가 이미 써낸 판단을 사람이 빠르게 훑어 맞는지 확인하고, 틀린 부분만 최소한으로 고치는 검수 표면이다 — 처음부터 채워 넣는 폼이 아니다." 사람의 기본 동작은 "쓰기"가 아니라 "읽고 판단하기"이고, 편집은 그 판단 과정에서 나오는 예외적 후속 동작이다.
- **카드 자체의 인식**: "독립된 완결 메모"(Zettelkasten 인덱스카드·포스트잇 더미 비유) — 각 후보는 서로 다른 판단(타입도 다르고 개별 삭제 가능)이라 경계는 필요하지만, 그 경계가 "폼 박스"처럼 보이면 안 된다. 그래서 카드마다 4방향 테두리 대신 **항목 사이 구분선만**(`divide-y divide-border-subtle`) — IngestionScreen에 프로토타입으로 반영, `DigestSection`/`ReferenceSection` 렌더는 임시로 주석 처리한 상태(본문 요소가 다 채워지기 전까진 빈 리스트로 구조만 검증 중).
- **작성 방식**(원칙만 확정, 미구현): 필드는 평소 이미 완성된 텍스트처럼 보이다가 클릭하면 그 자리에서 바로 편집(제자리 편집, 별도 편집 모드·상시 입력 상자 없음). 리치 텍스트 에디터는 지양 — 서식(볼드·헤딩 등)은 전혀 없고, 유일한 인라인 엔티티는 레퍼런스 멘션인데 이것도 "@" 버튼이 아니라 **텍스트를 드래그 선택하면 뜨는 오버레이**(선택 영역에 "레퍼런스로 연결" 액션 하나만)로 확정. surface-inventory.md가 "@ 버튼 방식"으로 적어둔 건 와이어프레임 속도용 임시 구현이라 무시하고 새로 정한 것.

**헤더 3단 위계 (구현 완료)**
- **Topic**(가장 조용함, 제목 위 브레드크럼) → **타입**(Chip pill) → **제목**(가장 무거움, `Text size="lg" weight="semibold"`). Topic 텍스트는 `fg-tertiary`, 색은 안 씀 — `TopicChipRow`가 기존에 쓰던 `variant="brand"`(틸)를 참고할까 했으나, 그것도 임시 구현이라 무시하고 처음부터 재검토한 결과 "이 자리에서 색이 뭔가를 구별해주지도, 모호함을 없애주지도 못한다"(위치로 이미 Topic임이 명확, 모든 Topic이 같은 색이라 항목 간 구별에도 무의미)는 결론.
- **타입 5종(결정/미결/학습/아이디어/가정) 전부 같은 중립색** — 5개가 좋다/나쁘다 축이 없는 순수 분류라 색을 배정하면 없는 의미가 생기고, 라벨 텍스트 자체(짧고 서로 다른 단어)로 이미 구별되며, 색을 쓰면 제목과 시각적 무게를 다투게 된다는 판단(design-reference-log.md ⑧ 참고).

**새 weave 컴포넌트/수정 (구현 완료)**
- **`Chip`**(`packages/weave/src/components/Chip.tsx`, 신규) — Badge와 짝을 이루는 인터랙티브 pill. `variant`(`neutral`/`outline`), `truncated` prop(min-w-0+truncate 조합을 캡슐화). Button을 안 쓰는 이유는 base가 `text-[13px] font-semibold`를 강제해 되돌리는 비용이 커서(weave-usage.md "칩·pill 안 버튼" 제외 규칙).
- **Badge `neutral` variant 다크 모드 버그 수정**: `bg-surface-raised`가 다크 모드에서 `surface-card`(카드 배경)와 완전히 같은 값이 돼(`tokens/index.css`) 카드 위에서 안 보이던 문제 — `bg-fg-primary/10`(상대적 알파 틴트라 배경이 뭐든 항상 대비 생김)으로 교체, `Chip`과 톤을 공유(`NEUTRAL_TONE_CLASSNAME` export). 새 토큰은 안 만듦 — 이미 토큰(`fg-primary`)의 표준 알파 모디파이어라 raw 값이 아니고, 소비처도 2곳뿐이라 팔레트·시맨틱·Tailwind 노출 3단 신설 절차를 거칠 실익이 없다고 판단.
- `DraftSpaceSelect`/`DraftSpaceLabel`(intake 기능)도 각각 `Chip`/`Badge`로 갈아탐 — 손으로 들고 있던 `SPACE_PILL_CLASSNAME`과 그 파일(`intake/constants.ts`) 삭제. `DraftSpaceLabel`은 `Chip`(button)이 아니라 `Badge`(span)로 — Working→Idle 전환 시 `disabled`가 풀리는 게 아니라 컴포넌트 자체가 통째로 교체되는 구조라, "임시로 막힌 버튼"이 아니라 "애초에 버튼이 아닌 자리"라서 `disabled` prop으로는 부정확한 시맨틱이 된다는 이유.
- **`LabelChipShell`**의 칩 제거(X) 버튼도 카드 삭제 버튼과 같은 원칙으로 기본 숨김·hover/focus-visible에만 노출(`transition-none` 포함) — `TagChipRow`/`TopicChipRow` 둘 다 공유하는 컴포넌트라 두 곳에 다 적용됨.

**타입·필드 라벨 (구현 완료)**
- `DIGEST_TYPE_LABEL`/`DIGEST_BODY_FIELDS`의 국문 하드코딩을 tolgee 키(`review.digest_type_*`, `review.digest_field_*`)로 전환. 영문 라벨을 이번에 처음 확정하면서 국문도 일부 재검토: 미결→**Unresolved**(Pending 대신, RAID/열린질문 뉘앙스 리서치 근거), 가정 타입 필드(`assumption`)는 국문 **가설**/영문 **Hypothesis**로(타입 배지=가정/Assumption과 구분), 갈래→**선택지**/Options(git 브랜치와 혼동 회피 + "다이제스트 후보" 기존 의미와 충돌 회피), 해소 조건→**확인 조건**/Confirmation condition(가정 타입의 "검증 조건"과 짝이 되는 표현으로 통일). 근거는 design-reference-log.md ⑧ 참고.

**Topic 피커 (구현 완료, 디테일은 미확정)**
- 평소 조용한 텍스트(hover 시 밑줄만, 배경 없음) → 클릭하면 팝오버에서 기존 `TopicChipRow` 그대로 노출. Topic 0개일 땐 브레드크럼 자체가 없어 추가 진입점이 사라지는 문제가 있어, **점선 outline `Chip` + Plus 아이콘만**(텍스트 없음, `aria-label`로 접근성 유지)으로 대체 — outline variant 자체는 실선이 기본이고, 점선은 이 소비처에서만 `className="border-dashed"`로 얹음(outline=채움 없음 vs 점선=빈 자리 신호는 별개 축이라 컴포넌트에 고정 안 함).
- **아직 안 정한 것**: Topic 여러 개일 때 구분자(지금 middot `·` 유지 vs 쉼표 `,` — 태그에 테두리가 없어졌으니 구분자가 그 역할을 대신해야 한다는 문제의식에서 나온 논의, 미결). 클릭 가능 영역이 지금 텍스트/`+` 버튼 크기만큼만이라 요소가 작을 때 클릭이 어려운 문제 — 행 전체로 히트박스 확장 필요(미구현). 팝오버가 지금 Radix 기본 위치(아래로 밀려서 뜸)인데, Notion처럼 트리거 자리를 덮으며 뜨는 느낌으로 포지셔닝 조정 필요(미확인 — 브라우저로 실측 먼저 필요). 팝오버 내부(`TopicChipRow` 그대로 재사용 중)도 Notion 레퍼런스(선택된 칩이 인풋처럼 상단 + 구분선 + 라벨 + 검색 가능한 전체 목록)로 재정비할지 논의 중.

**전혀 착수 안 한 것**
- 레퍼런스 카드(원래 작업 순서상 다이제스트 다음 차례, 아직 시작 안 함).
- 본문 구조화 필드(상황/선택/이유 등) 실제 렌더링·편집 UI — 원칙(조용한 라벨, 인라인 멘션)만 정했고 구현 없음.
- 확정 후 화면 — `ChangesetRecordScreen`이 헤더만 그리고 Digest·Reference 스냅샷을 아예 안 그림(세션 초반 발견한 gap, 손 안 댐).
- 페이지 전체 레이아웃(카드 간 배치, 원문 사이드패널과의 관계) — 작업 순서상 ②단계, 아직 ①(카드 구조)에 머무름.

관련 커밋: `4dea338e`(카드 바탕 프로토타입 + 타입·필드 라벨 i18n), `f7b7b589`(weave `Chip` 신설 + Space·타입 pill 통일).

---

### 2026-07-22 — Topic 팝오버 확정, 타입을 헤더에서 메뉴로, 읽음(접힘)·타입 색상 팔레트 신설 (wt-3 `polish/changeset-detail-body`, 인수인계 계속)

바로 위 섹션의 "아직 안 정한 것"·"전혀 착수 안 한 것" 중 Topic 피커·헤더 위계 부분을 마무리하고, 이번 라운드에서 새로 나온 결정들.

**Topic 팝오버 최종 형태**: `TopicChipRow` 재사용 대신 `TopicEditPanel` 신설 — 칩 목록과 검색 인풋을 한 박스에 같이 두는 Notion 프로퍼티 방식(칩 목록 → 구분선 → 검색 리스트). 팝오버는 트리거 자리를 완전히 덮으며 뜬다(`sideOffset={-24}`). 검색 후보는 텍스트가 아니라 실제로 생성될 모양 그대로 **Badge 미리보기**로 보여준다(선택 전에 결과를 예고). "새로 만들기" 문구는 tolgee `t()`가 문자열 파라미터만 받아 Badge를 문장 중간에 못 끼워서, 번역 키를 앞/뒤(`_before`/`_after`)로 쪼개 그 사이에 Badge를 직접 렌더링하는 방식으로 우회했다 — 언어별 어순 차이(국문은 뒤, 영문은 앞에 문구가 붙음)도 이 구조로 자연히 처리된다. 로딩 상태는 스피너·텍스트가 아니라 실제 후보 행 모양의 `Skeleton`(폭 제각각)으로 — 스피너 대신 스켈레톤 원칙(`DraftSpaceSelect`와 동일). 빈 상태 문구는 "일치하는 항목이 없어요"(진짜 검색 실패)와 "이미 모두 추가했어요"(Space에 Topic은 있는데 다 골라서 후보가 0개)를 원인별로 분리 — 전자를 후자 상황에 그대로 쓰면 "검색이 실패했다"는 잘못된 신호가 된다. 리스트 행 hover 톤은 `DropdownMenuItem`의 실제 값(`/75` 라이트·100% 다크)에 맞췄다 — 기존 코드는 라이트에서도 100%를 써서 앱의 실제 메뉴 행 표준보다 진했다.

**Topic 텍스트 표현**: 색을 `fg-tertiary`→`fg-primary`로 올렸다(제목·본문 값과 같은 명도, 가독성 문제 해결 — 굵기·크기는 그대로 둬 위계는 유지). 복수 Topic 구분자는 텍스트 글리프 `·`가 12px에서 잉크가 너무 적어 안 보여, 본문 리스트 불릿과 같은 Circle 아이콘(`fill-current`)으로 교체. hover 밑줄은 `text-decoration`이 아니라 버튼 전체의 `border-b`로 — flex로 쪼개진 자식(각 Topic span·Circle)마다 데코레이션이 끊겨 그려지는 문제가 있어서다. `border-b`가 늘린 박스 높이(1px)는 `-mb-px`로 상쇄해 옆 요소(타입·미트볼)와 나란히 있을 때 미세하게 떠 보이지 않게 했다. Topic엔 값별 자동 색상 할당(Notion 태그처럼)을 안 준다 — 그 팔레트는 태그(Tag) 쪽에 이미 예약돼 있어 Topic과 톤이 겹치면 안 된다.

**타입(Type)을 헤더에서 완전히 뺐다 — 미트볼 서브메뉴로 이동**. 여러 위치를 시도했었다(Topic 왼쪽 리딩 → 우측 고정 슬롯[MD3 leading/text/trailing 근거] → 다시 Topic 왼쪽). 최종적으로 헤더 상시 노출 자체를 접었다 — 근거 둘: (1) 타입별 전용 필드 라벨(상황/선택 vs 질문/선택지 등)이 본문에서 이미 타입을 드러내 헤더에 중복 표시할 필요가 없고, (2) 타입 변경이 그 타입 전용 필드를 전부 날리는 파괴적 동작이라 "AI가 잘못 분류했을 때 이미 정답을 알고 바로잡는" 드문 교정 동작에 가깝다(→ 삭제와 같은 층위, 미트볼 메뉴가 맞는 자리). 서브메뉴는 교체형 드릴다운이 아니라 **플라이아웃**(Radix `Sub`) — 데스크톱 드롭다운의 표준 패턴이고(Windows 앱 디자인 가이드도 명시), 우리 화면은 마우스·hover 전제라 모바일 전용인 드릴다운을 쓸 이유가 없다. 선택 표시는 `DropdownMenuRadioItem` 기본값(좌측 점)이 아니라 **우측 체크마크**로 맞췄다 — 이 앱은 "지금 선택된 값"을 이미 `Select`·`TopicAddPopover`에서 우측 체크로 통일해뒀다. 라벨은 "타입 변경"이 아니라 **"유형 변경"** — 이 앱이 이미 `digest_type_label`("다이제스트 유형") 등에서 "유형"을 쓰고 있어 "타입"은 그 용어와 안 맞는다.

**읽음(접힘) 기능 신설**: GitHub PR의 Viewed 체크박스와 같은 동작 — 체크하면 본문 필드 섹션 자체를 그리지 않고 헤더(Topic·제목·description)만 남긴다. 스레드 피드가 아직 없는 상태에서, 이 "접힌 모습"이 미래 피드 행의 시각적 예고편이 되도록 의도했다(design-reference-log.md ⑨·⑩·⑪, "헤더 워시 = 피드로 나갈 미리보기 단위"라는 옆 세션의 독립적 결론과도 부합). 컨트롤은 weave `Checkbox`(실제 체크박스, 키보드·스크린리더 지원)를 쓰되 겉모습만 `buttonVariants`를 raw label에 입혀 눌린 버튼처럼 보이게 했다 — `Checkbox` 자체가 Radix에서 `<button>` 태그라 weave `Button`으로 감싸면 button 중첩(무효한 HTML)이 되어 이 조합이 필요했다. 읽음 상태에서는 본문이 사라져 타입을 유추할 단서도 같이 사라지므로, 그때만 헤더에 타입 **Badge**를 다시 노출한다(Topic 왼쪽 리딩 위치 — 우측은 지금 액션 버튼들이 차지 중이라 자리가 없고, 나중에 액션이 없는 실제 피드 화면에서는 우측 고정 슬롯이 더 맞을 수 있음, 화면마다 다르게 가도 됨). 접힘 상태의 카드는 짧아진 만큼 카드 간 여백도 좁힌다 — 다만 카드마다 위·아래 여백을 다 갖던 구조를 버리고, **리스트 래퍼에 `pt-4`(페이지 헤더와의 간격) 하나만 고정으로 두고 각 카드는 `pb`만**(펼침 `pb-8`/접힘 `pb-4`) 갖게 바꿨다 — 그래야 개별 카드가 접혀도 페이지 헤더와의 간격이 흔들리지 않는다(안 그러면 리스트 맨 위 카드를 접을 때 레이아웃 시프트가 났다). **접힘 애니메이션(트랜지션)과 카드별 sticky 헤더는 검토 후 안 하기로 결정** — 후자는 이 카드가 GitHub 파일 diff와 달리 뷰포트보다 짧아서(구조화 필드 5개 안팎) sticky가 풀어야 할 문제 자체가 잘 안 생기고, 드물게 카드가 길어지는 경우도 읽음 접기로 이미 커버된다.

**Digest 타입 5종 전용 색상 팔레트 신설(weave)**: `digest-type-{decision,pending,learning,idea,assumption}`(+tint)를 팔레트·시맨틱·Tailwind 3단 전부(라이트/다크/인버티드 4개 테마 블록) 추가하고, `Badge`에 `digest-*` prefix variant 5개를 추가했다. Topic엔 색을 못 준 이유("유저가 무한히 만드는 개방형 라벨이라 값별 매핑이 스케일 안 됨")가 타입엔 안 걸린다 — 타입은 제품이 정한 5개 고정 집합이라, 그 이유로 기각했던 "뉴스 카테고리처럼 작고 고정된 집합" 조건에 정확히 들어맞는다(design-reference-log.md ⑪). 다만 기존 상태 토큰(brand/success/warning/error/info)과는 색상 축 자체를 겹치지 않게 새로 골랐다 — 재사용하면 앞서 있었던 "AI 작업중 블루를 다른 의미로 재사용" 문제가 반복된다. variant 이름에 `digest-` 접두사를 둔 이유도 같다 — 이 앱의 다른 Badge 소비처가 실수로 가져다 쓰는 걸 막는다. **텍스트는 배경과 같은 색으로 진하게(status 톤과 동일 공식)** — 이건 미래에 만들 Tag(유저가 무한히 만드는 open-ended 태그)에는 안 맞는 방향이라는 논의가 있었다: Tag는 화면 하나에 색 다른 태그가 여러 개 동시에 뜰 수 있어 텍스트까지 진하면 시끄러워지므로, Notion처럼 **배경만 색 + 텍스트는 중립**으로 가는 게 스케일에 맞다는 방향까지만 합의했고 미구현이다(타입 5종은 값이 고정이라 반대로 색이 또렷이 읽혀야 하는 쪽이 맞다고 보고 유지).

**weave `Alert`도 텍스트를 중립(`fg-primary`)으로 바꿨다** — 배경 틴트·아이콘 색은 유지. `Badge`가 이미 "틴트 배경+같은 색 텍스트"를 자기 시각 언어로 쓰고 있어서, `Alert`까지 텍스트를 색칠하면 두 컴포넌트가 구분이 안 된다는 문제의식(Badge=값 하나를 표시하는 작은 태그, Alert=배너)에서 나온 컴포넌트 레벨 변경 — 기존 7개 사용처 전부에 영향.

**mutation 실패 처리 컨벤션 정리**: 리뷰·초안·계정 삭제 플로우 여러 곳에 "이 화면이 인라인 Alert로 실패를 보여주니 전역 토스트는 끈다"(`meta.skipGlobalToast`)는 관례가 있었는데, 점검해보니 대부분 인라인으로 보여줄 실익이 없는 임시 구현이었다(`useUpdateReview`/`useConfirmReview`/`useDiscardReview`, `useUpdateSourceTitle`/`useUpdateSourceBody`/`useStartSourceDigestion`) — 전역 토스트로 전환. 단 `useDeleteAccount`는 예외로 남겼다 — PRECONDITION_FAILED(소유권 이전 필요)만 화면이 직접 안내(원인별 문구 + 차단 목록 refetch)해야 해서 그 경우만 `skipGlobalToast`를 유지하고, 나머지 실패는 같은 `onError` 안에서 직접 `toastError`를 호출해 전역 토스트와 같은 결과를 낸다 — 이런 "부분적으로만 인라인이 필요한" 케이스는 `skipGlobalToast` on/off 하나로 못 나누고 훅 내부에서 분기해야 한다는 게 이번에 확인된 패턴. dev-harness의 동일 패턴 6곳은 프로덕션 화면이 아니라는 이유로 스코프 밖으로 남겨뒀다(미검토 아님, 의도적 보류).

**아직 안 정한 것**
- 참조 멘션(텍스트 드래그 선택 → 오버레이 액션)은 원칙만 확정된 채 여전히 미구현 — 직렬화 방식이 백엔드와 얽혀 있어 보류 중.
- Tag 구현 시 "배경만 색 + 텍스트 중립" 방향(위 참고)은 합의만 됐고 미구현.
- 스레드 피드 자체는 미착수 — 이번 라운드의 결정 다수(타입 Badge 위치, 헤더=피드 미리보기 단위)가 그 화면을 겨냥해 내려졌으므로, 실제로 만들 때 여기 근거를 먼저 참고할 것.

**전혀 착수 안 한 것** (이전 섹션에서 이어짐, 여전히 유효)
- 레퍼런스 카드 재설계.
...
### 2026-07-23 — Topic·Tag 부착/해제 진입점 통일, 이름·설명 수정 팝오버 신설 (PR #478, wt-3 `polish/digest-card-followups`)

바로 위 섹션의 "전혀 착수 안 한 것" 중 레퍼런스 카드 재설계와는 별개로 진행된 Topic·Tag 편집 UX 한 라운드 전체(레퍼런스 카드 재설계 자체의 결정은 바로 다음 섹션 참고).

**Topic·Tag 부착/해제 진입점을 미트볼 하나로 통일**: 기존엔 선택된 항목이 검색 목록에서 사라지는 방식이었는데, "떼어내기"만을 위해 미트볼 메뉴를 따로 추가하면 진입점이 두 개(목록 클릭=부착, 미트볼=해제)로 갈려 헷갈린다는 문제의식에서, 선택된 항목도 목록에 그대로 노출하되 클릭으로는 재선택되지 않게(`aria-disabled`/`disabled`) 하고 우측에 상시 노출 미트볼을 둬 부착 여부와 무관하게 진입점을 하나로 합쳤다. attached 여부는 Badge 톤 차이로만 구분(별도 배지·아이콘 추가 없음).

**이름·설명 수정은 별도 확인(컨펌 모달) 없이 인라인으로** — `07-modeling.md`가 명시한 대로 Topic·Tag는 changeset 대상이 아닌 "찾기용 라벨"이라 잘못 바뀌어도 판단을 오염시키지 않으므로, 삭제(별도 컨펌 필요)와 달리 수정은 즉시 반영해도 된다는 원칙을 그대로 적용했다.

**수정 팝오버 최종 형태**: 노션의 "메뉴처럼 보이다가 인풋으로 바뀌는" 패턴을 참고했으나 그대로 가져오진 않았다 — Tag는 설명이 필수 필드라 노션처럼 접어뒀다 아이콘으로 펼치는 방식은 안 맞고, 이름·설명 모두 팝오버 진입 즉시 상시 노출한다. 다만 레이블·버튼을 다 빼고 나니 팝오버 자체가 통째로 인풋처럼 보이는 문제가 있어, 이름·설명 각각을 팝오버 내부의 테두리 있는 원라인 `<input>`으로 감싸 "이 안에 입력 필드가 있다"는 시각적 신호를 남겼다. 적용 방식은 apply-on-close — 별도 저장 버튼 없이 Enter 또는 팝오버가 닫히는 모든 경로(바깥 클릭, Esc, 코드로 닫기)가 `applyAndClose` 한 곳으로 모여, 값이 실제로 바뀐 경우에만 mutate한다.

**미트볼 hover 패턴을 앱 표준에 맞춤**: `SpaceItemMenu`/`LnbHoverIcon` 등 기존 리스트 행의 호버 배경(`LIST_ITEM_HOVER_CLASSNAME`)·미트볼 자체 호버(`NESTED_HOVER_ICON_CLASSNAME`) 상수를 그대로 재사용해, 새로 만든 Topic·Tag 리스트도 다른 화면과 같은 호버 톤을 쓰게 했다. Tooltip과 팝오버 트리거를 같은 엘리먼트에 `asChild` 이중으로 얹지 않고 분리했다 — `SpaceItemMenu`에 이미 기록된 Radix `data-state` 충돌 버그를 피하기 위함.

**태그 생성 폼**: 이름은 (직전 단계에서 이미 확정돼) 수정 불가한 정적 Badge로 보여주고 바로 설명 필드에 오토포커스, 설명 placeholder는 제거(빈 상태 자체가 충분한 신호). "생성"/"뒤로" CTA 문구를 `common.create`/`common.back` 공용 키로 승격해 계정 삭제 차단 화면 등 기존 사용처도 같은 키로 통일했다.

**인풋 다크모드 포커스 보더 수정**: 새로 만든 팝오버 인풋들이 weave `Input`의 `focus-visible:border-brand dark:focus-visible:border-fg-tertiary/70` 컨벤션을 안 따르고 있던 걸 발견해 맞췄다.

**아직 안 정한 것** (이전 섹션에서 이어짐, 여전히 유효)
- 참조 멘션(텍스트 드래그 선택 → 오버레이 액션) — 여전히 미구현.
- Tag "배경만 색 + 텍스트 중립" 방향 — 이번 라운드에서도 안 건드림, 합의만 된 채 미구현.
- 스레드 피드 — 미착수.

관련 커밋: `bb66b434`(PR #478, squash merge — "태그·주제 이름/설명 수정 기능 추가", "'뒤로' 문구를 공용 번역 키로 승격", "태그 생성 폼 진입 시 기존 칩 목록·구분선 숨김", "태그 검색 후보에 설명 툴팁 추가", "PR 리뷰 반영 — 태그·주제 rename 전파, 중첩 팝오버 스크롤, 접근성·검증 공백" 등 세부 커밋 메시지 참고).
