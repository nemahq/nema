...
**검증**: 로컬 Supabase에 `digestion_status` 4값(pending/failed/cancelled/completed) × rejected changeset 유무 조합으로 원본 4개를 시딩해 `source.listPending`을 직접 호출 — `digestionOutcome`이 `processing`/`failed`/`cancelled`/`empty`/`discarded` 각각 정확히 나오는 것을 API 응답으로 확인. `listPendingSources`에 `digestion_status`별 매핑 유닛 테스트 추가(`rejected` 이력이 있어도 `completed`가 아니면 `discarded`로 새지 않는 것까지 포함). `pnpm typecheck`/`lint`/`test`/`format:check`/`knip`/`depcruise` 모노레포 전체 통과 — 특히 dev-harness `statusLabel()`의 새 `switch`가 5값을 빠짐없이 처리하는지는 `noImplicitReturns` 컴파일 자체가 검증(값 하나만 빠뜨려도 타입 에러).
