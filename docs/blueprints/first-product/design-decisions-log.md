...
**구현**:
- `usePendingAfterDelay(isPending, delayMs=250)`(`apps/web/src/lib/tanstack-query/usePendingAfterDelay.ts`) — `isPending`이 지연 시간 넘게 유지될 때만 `true`를 반환하는 순수 타이머 훅(구현 자체엔 tRPC/데이터 인식 없음). `useMutation`의 유일한 내부 소비처라 같은 폴더에 둔다 — "언젠가 툴팁 hover-intent나 애니메이션 지연에도 재사용할 수 있으니 feature-agnostic한 `hooks/`에 둬야 한다"는 초기 판단은 근거 없는 가정이었음이 드러나 기각(툴팁 지연은 Radix `Tooltip`의 `delayDuration` prop으로 이미 별도 해결돼 있고, Nema에 이 훅이 실제로 필요한 비-tRPC 비동기 작업 사례가 없음 확인). 이 훅의 존재 이유 자체가 데이터 페칭/변형 UX 디테일이라, `lib/` 안에 `useMutation`과 나란히 두는 게 맞다.
