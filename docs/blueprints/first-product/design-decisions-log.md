...
- **i18n**: `intake.draft_title_placeholder`/`draft_title_edit_action`/`draft_title_edit_label`/`draft_title_edit_title`/`draft_title_edit_required` 신규, en 실작성 + ko는 intake 네임스페이스 기존 관례대로 en과 동일 placeholder. Save/Cancel/Saving은 새 키를 안 만들고 기존 `common.save`/`common.cancel`/`common.saving` 재사용(PR #391의 "action 라벨은 common으로 통합" 결정 계승).
